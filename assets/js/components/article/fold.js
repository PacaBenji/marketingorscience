...
(function () {
  ...
  function buildEvidenceBar(rating, max) {
    ...
    for (var i = 1; i <= max; i++) {
      segs +=
        '<span class="grade-bar-segment' +
        (i <= rating ? " filled" : "") +
        '"></span>';
    }
    return (
      '<span class="grade-bar" role="img" aria-label="Evidence strength ' +
      rating +
      " of " +
      max +
      '">' +
  ...
  function createFold(config) {
    ...
    if (cfg.verdict) {
      ...
      verdictHTML =
        '<div class="verdict-block verdict-block--' +
        state +
        '">' +
      ...
      if (cfg.verdict.note) {
        verdictHTML += '<p class="verdict-note">' + cfg.verdict.note + "</p>";
      }
    }
    ...
    // Optional tags[] replaces the default category + type badge row
    var metaBadgesHTML = "";
      ...
      metaBadgesHTML =
        cfg.tags
          .map(function (label, i) {
            ...
            return '<span class="article-type-badge">' + label + "</span>";
          })
          .join("") + peerBadge;
    } else {
      metaBadgesHTML =
    ...
    var shareFoldHTML =
      ...
      '<button class="share-btn share-btn--copy-fold" data-url="' +
      safeUrl +
      '">' +
      ...
      "Copy Link" +
      "</button>" +
      '<a class="share-btn" href="https://twitter.com/intent/tweet?url=' +
      encodedUrl +
      "&text=" +
      encodedTitle +
      ...
      "LinkedIn" +
      "</a>" +
      '<a class="share-btn" href="mailto:?subject=' +
      encodedTitle +
    ...
    if (cfg.correction && cfg.correction.date && cfg.correction.text) {
      correctionHTML =
        ...
        '<p class="correction-notice-label">' +
        (cfg.correction.label || "Correction") +
        " \u2014 " +
        formatDate(cfg.correction.date) +
        "</p>" +
        "<p>" +
        cfg.correction.text +
        "</p>" +
    ...
    var belowInnerHTML =
      ...
      (cfg.deck ? '<p class="fold-deck">' + cfg.deck + "</p>" : "") +
    ...
    if (heroSrc) {
      ...
      belowHTML =
        '<div class="fold-inner">' + belowInnerHTML + correctionHTML + "</div>";
    }
