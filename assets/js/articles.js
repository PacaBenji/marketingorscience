// articles.js — marketingorscience.com
// Central article manifest. Powers search, read-more, category filter, and author pages.
// Exposed as window.MOS_ARTICLES.

window.MOS_ARTICLES = [
  {
    slug         : 'aod-9604-fat-loss-peptides',
    relatedSlugs : ['what-are-peptides-explainer', 'compounded-glp1-semaglutide-tirzepatide', 'peptide-compounding-fda-review'],
    url          : '/articles/pharma/2026/09/aod-9604-fat-loss-peptides',
    title        : 'AOD-9604 and Fat-Loss Peptide Fragments: The “Ozempic Alternative” Without the Evidence',
    deck         : 'AOD-9604 is marketed as a fat-targeting peptide that delivers GLP-1-like results without the side effects. The clinical trial history — including a failed Phase 2b that its current marketing ignores — tells a different story.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Clinical Review',
    tags         : ['aod-9604', 'hgh-fragment', 'fat-loss', 'peptides', 'weight-loss', 'compounding', 'glp1', 'ozempic-alternative', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-21',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'aod-9604 hgh fragment 176-191 fat loss peptide ozempic alternative weight loss compounding lipolytic metabolic pharmaceuticals phase 2b failed clinical trial placebo no difference animal data human evidence gap',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/peptide-vials.jpg',
  },
  {
    slug         : 'what-are-peptides-explainer',
    relatedSlugs : ['aod-9604-fat-loss-peptides', 'beauty-peptides-evidence-review', 'collagen-peptides'],
    url          : '/articles/wellness/2026/09/what-are-peptides-explainer',
    title        : 'What Are Peptides? What the Label Actually Means',
    deck         : '“Contains peptides” appears on thousands of products. A plain-language guide to what peptides actually are, why delivery route determines whether they can work, and why the word alone tells you almost nothing.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Explainer',
    tags         : ['peptides', 'bioavailability', 'skin-penetration', 'oral-peptides', 'injectable', 'cosmetics', 'supplements', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-18',
    verdict      : null,
    verdictClass : null,
    searchText   : 'what are peptides amino acids peptide bonds molecular weight dalton rule skin penetration oral bioavailability injectable delivery route stratum corneum digestion first pass metabolism synthetic bioidentical naturally derived cosmetics drug claim mocra label',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/women-peptides-skin.jpg',
  },
  {
    slug         : 'before-start-minoxidil',
    relatedSlugs : ['minoxidil-non-responders', 'minoxidil-vs-novogro', 'minoxidil-female-pattern-hair-loss'],
    url          : '/articles/haircare/2026/09/before-start-minoxidil',
    title        : 'Before You Start Minoxidil: 5 Things to Know',
    deck         : 'How to tell if minoxidil is likely to work, what the early shedding actually means, which side effects warrant stopping, and how a newer minoxidil-free approach compares.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Clinical Review',
    tags         : ['minoxidil', 'sult1a1', 'hair-loss', 'reyou', 'novogro', 'side-effects', 'non-responder', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-17',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'before you start minoxidil five things to know sult1a1 sulfotransferase enzyme non-responder shedding side effects facial hair scalp irritation when to stop dual-path reyou novogro nv-623 nv-624 nv-273 dermal papilla minoxidil-free clinical review',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/before-start-minoxidil-img2.1.png',
  },
  {
    slug         : 'meno-hair-serums',
    relatedSlugs : ['finasteride-vs-novogro', 'low-dose-oral-minoxidil', 'minoxidil-female-pattern-hair-loss'],
    url          : '/articles/haircare/2026/09/meno-hair-serums',
    title        : 'The Science Behind Meno-Hair and Four Hair Serums that Can Help',
    deck         : 'A lot changes during menopause, does your hair have to be a part of it too? We looked at the so-called “best” hair serums for menopausal hair loss — here’s what the science actually says.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['menopause', 'hair-loss', 'reyou', 'vegamour', 'minoxidil', 'oneskin', 'novogro', 'hair-serum', 'fphl', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-10',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'meno-hair menopause perimenopause hair thinning serum reyou vegamour hers minoxidil oneskin os-01 novogro dual-path revival estrogen dht scalp circulation hair loss female pattern treatment analysis',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/menopause-lp-hero.png',
  },
  {
    slug         : 'dust-allergens-der-p1-der-p2',
    relatedSlugs : ['ceramide-moisturizers-skin-barrier', 'baby-eczema-condition-review', 'dandruff-scalp-microbiome'],
    url          : '/articles/haircare/2026/07/dust-allergens-der-p1-der-p2',
    title        : 'Your Skin vs. Dust Allergens: The Disruptive Effects of Der p 1 and Der p 2',
    deck         : 'Der p 1 cleaves barrier proteins. Der p 2 amplifies immune signaling. A review of both mechanisms and what EnviroBlock™ would need to show to claim dual protection against house dust mite allergens.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['der-p-1', 'der-p-2', 'dust-mites', 'skin-barrier', 'enviroblock', 'resveratrol', 'atopic-dermatitis', 'allergens', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-18',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'der p 1 der p 2 house dust mite allergens skin barrier tight junction claudin protease tlr4 th2 atopic dermatitis eczema enviroblock resveratrol barrier protection environmental allergen ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/dust_vs_skin_hero.png',
  },
  {
    slug         : 'hair-growth-supplements-nutrafol-viviscal',
    relatedSlugs : ['biotin-hair-growth', 'iron-deficiency-hair-loss', 'saw-palmetto-hair-loss'],
    url          : '/articles/haircare/2026/08/hair-growth-supplements-nutrafol-viviscal',
    title        : 'Hair Growth Supplements (Nutrafol, Viviscal, Biotin): What the Evidence Shows',
    deck         : 'Nutrafol, Viviscal, and biotin gummies are marketed as clinically proven hair-loss solutions. A review of the trial design problems — manufacturer funding, placebo-only benchmarks, and soft endpoints — and what the evidence actually supports.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['nutrafol', 'viviscal', 'hair-supplements', 'biotin', 'aminomar', 'saw-palmetto', 'ashwagandha', 'hair-growth', 'hair-loss', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-27',
    verdict      : 'Claim Overstated',
    verdictClass : 'partial',
    searchText   : 'nutrafol viviscal biotin hair growth supplements gummies clinically proven thinning hair synergen complex ashwagandha saw palmetto marine collagen tocotrienols aminomar marine complex shark mollusk powder ablon 2018 2021 rizer 2015 randomized double blind placebo controlled rct manufacturer funded industry funded small trial soft endpoints self perceived thinning phototrichoscopy vellus terminal hair count shedding no drug comparison no minoxidil finasteride benchmark dietary supplement not fda approved deficiency rare biotin interference assay iron deficiency ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/hair-growth-supplements.jpg',
  },
  {
    slug         : 'exosome-stem-cell-hair-loss',
    relatedSlugs : ['prp-hair-loss-evidence', 'exosome-skincare', 'copper-peptides-hair-growth'],
    url          : '/articles/haircare/2026/09/exosome-stem-cell-hair-loss',
    title        : 'Exosomes, Stem Cells & Hair Cloning: What the Evidence Actually Shows',
    deck         : 'Exosome injections, stem-cell therapies, and hair cloning are the frontier of regenerative hair loss treatment. A review of what each approach actually is, where the science stands, and why none of these are ready to replace established treatments.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['exosomes', 'stem-cells', 'hair-cloning', 'onion-juice', 'pumpkin-seed-oil', 'prp', 'regenerative', 'hair-loss', 'androgenetic-alopecia', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-12',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'exosomes stem cells hair cloning regenerative hair loss dermal papilla wnt signaling extracellular vesicles growth factors platelet rich plasma prp fda warning unapproved biologic injectable scalp therapy stemson hair follicle neogenesis organ culture organoid onion juice alopecia areata pumpkin seed oil 2014 rct androgenetic alopecia finasteride minoxidil viral diy tiktok reddit not ready established treatment frontier early science small trial',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/exosome-stem-cell-hair.jpg',
  },
  {
    slug         : 'prostaglandin-analogs-hair-loss',
    relatedSlugs : ['minoxidil-female-pattern-hair-loss', 'copper-peptides-hair-growth', 'red-light-therapy-hair-growth'],
    url          : '/articles/pharma/2026/09/prostaglandin-analogs-hair-loss',
    title        : 'Prostaglandin Analogs (Latanoprost, Bimatoprost) for Scalp Hair Loss',
    deck         : 'Bimatoprost grows eyelashes — does it regrow scalp hair? A review of the prostaglandin pathway in hair biology, the small scalp trials for latanoprost and bimatoprost, and the largely stalled PGD2 hypothesis.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Treatment Analysis',
    tags         : ['prostaglandin', 'latanoprost', 'bimatoprost', 'latisse', 'pgd2', 'setipiprant', 'hair-loss', 'off-label', 'androgenetic-alopecia', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-11',
    verdict      : 'Mixed',
    verdictClass : 'mixed',
    searchText   : 'prostaglandin analogs latanoprost bimatoprost latisse eyelash hypotrichosis scalp hair loss androgenetic alopecia pgf2alpha fp receptor anagen phase pgd2 dp2 crth2 inhibit follicle miniaturization setipiprant hair cloning small trial pilot rct blume-peytavi 2012 alopecia areata guo 2023 systematic review meta-analysis off-label not fda approved glaucoma eye drop penetration delivery eyelash vs scalp follicle',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/prostaglandin-hair-loss.jpg',
  },
  {
    slug         : 'dutasteride-vs-finasteride-hair-loss',
    relatedSlugs : ['dht-blockers-evidence', 'finasteride-female-hair-loss', 'saw-palmetto-hair-loss'],
    url          : '/articles/pharma/2026/09/dutasteride-vs-finasteride-hair-loss',
    title        : 'Dutasteride vs. Finasteride for Hair Loss: Is "Stronger" Actually Better?',
    deck         : 'Dutasteride blocks both type I and type II 5-alpha-reductase; finasteride blocks mainly type II. A review of the head-to-head evidence, the regulatory gap, and why greater DHT suppression doesn\'t automatically make it the right first choice.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Treatment Analysis',
    tags         : ['dutasteride', 'finasteride', '5-alpha-reductase', 'dht', 'androgenetic-alopecia', 'hair-loss', 'avodart', 'off-label', 'bph', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-10',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'dutasteride finasteride hair loss androgenetic alopecia 5-alpha-reductase type 1 type 2 dual inhibition dht suppression 90 95 percent 70 75 percent olsen 2006 head to head trial randomized placebo controlled gubelin harcha zhou 2019 meta-analysis bagatin 2024 hair count miniaturization reversal fda approved bph avodart propecia off-label usa korea japan half-life 5 weeks 6 8 hours side effects sexual gynecomastia stronger better upgrade',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/dutasteride-finasteride.jpg',
  },
  {
    slug         : 'spironolactone-female-hair-loss',
    relatedSlugs : ['finasteride-female-hair-loss', 'minoxidil-female-pattern-hair-loss', 'dht-blockers-evidence'],
    url          : '/articles/haircare/2026/09/spironolactone-female-hair-loss',
    title        : 'Spironolactone for Female Hair Loss: What the Evidence Supports',
    deck         : 'Spironolactone is widely prescribed off-label for female pattern hair loss. A review of the anti-androgen mechanism, what the observational and trial evidence shows, and the key cautions that make it women-only in practice.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['spironolactone', 'female-hair-loss', 'female-pattern-hair-loss', 'anti-androgen', 'aldosterone', 'off-label', 'androgenetic-alopecia', 'dht', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-09',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'spironolactone female hair loss female pattern hair loss androgenetic alopecia anti-androgen aldosterone antagonist diuretic androgen receptor block dht testosterone follicle miniaturization off-label not fda approved sinclair 2005 retrospective observational meta-analysis 56.6 percent improvement combination minoxidil placebo controlled trial lacking teratogen pregnancy contraindication gynecomastia men electrolyte potassium monitoring 100 200 mg postmenopausal contraception',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/spironolactone-hair-loss.jpg',
  },
  {
    slug         : 'caffeine-shampoo-hair-loss',
    relatedSlugs : ['minoxidil-female-pattern-hair-loss', 'copper-peptides-hair-growth', 'saw-palmetto-hair-loss'],
    url          : '/articles/haircare/2026/09/caffeine-shampoo-hair-loss',
    title        : 'Caffeine Shampoo (Alpecin) for Hair Loss: What the Evidence Shows',
    deck         : 'Caffeine shampoos like Alpecin promise to "wake up" follicles and fight hair loss. A review of the in-vitro data, the contact-time problem that undermines a rinse-off product, and why regulators banned the hair-loss claim.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['caffeine-shampoo', 'alpecin', 'caffeine', 'hair-loss', 'dht', 'follicle', 'contact-time', 'androgenetic-alopecia', 'minoxidil', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-09-01',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'caffeine shampoo alpecin hair loss regrowth wake up follicle energize scalp penetration contact time rinse off two minutes leave on liquid serum fischer 2007 in vitro human hair follicle proliferation testosterone androgenetic alopecia dhurat 2017 phyto-caffeine female systematic review 2025 adenosine advertising standards authority asa 2018 banned reduce hair loss claim substantiate cosmetic not fda approved minoxidil comparator saw palmetto copper peptides ingredient analysis',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/caffeine-shampoo.jpg',
  },
  {
    slug         : 'glp1-hair-shedding-timeline',
    relatedSlugs : ['ozempic-hair-loss-glp1', 'compounded-glp1-semaglutide-tirzepatide', 'telogen-effluvium-treatment'],
    url          : '/articles/pharma/2026/08/glp1-hair-shedding-timeline',
    title        : 'GLP-1 Hair Shedding Timeline: What to Expect After Weight Loss',
    deck         : 'Hair shedding after GLP-1 weight loss follows a predictable timeline. When it starts, when it peaks, how long recovery takes, and what the evidence shows.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Clinical Review',
    tags         : ['glp1', 'ozempic', 'semaglutide', 'tirzepatide', 'wegovy', 'zepbound', 'hair-shedding', 'hair-loss', 'telogen-effluvium', 'weight-loss'],
    author       : 'Dr. Brian Huber',
    authorSlug   : 'brian-huber',
    date         : '2026-08-22',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'glp1 hair shedding timeline glp-1 hair loss timeline how long hair shedding after glp-1 weight loss ozempic wegovy zepbound mounjaro semaglutide tirzepatide when does hair loss start peak recovery telogen effluvium rapid weight loss nutrient deficiency protein bariatric surgery months 2 4 6 12 anagen recovery reversible pharma clinical review',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/glp1-hair-timeline.jpg',
  },
  {
    slug         : 'minoxidil-non-responders',
    relatedSlugs : ['minoxidil-female-pattern-hair-loss', 'topical-vs-oral-minoxidil', 'low-dose-oral-minoxidil'],
    url          : '/articles/haircare/2026/08/minoxidil-non-responders',
    title        : 'Why Minoxidil Doesn\'t Work for Everyone: The Enzyme Behind Non-Response',
    deck         : 'Minoxidil fails for 20–40% of users. The main reason is a scalp enzyme, SULT1A1, that must convert the drug into its active form — plus the avoidable reasons treatment only appears to fail.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['minoxidil', 'non-responder', 'sulfotransferase', 'sult1a1', 'prodrug', 'minoxidil-sulfate', 'androgenetic-alopecia', 'hair-loss', 'tretinoin', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-12',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'minoxidil does not work for everyone non-responder non responder failure why fails sulfotransferase enzyme sult1a1 sult1a3 prodrug minoxidil sulfate active form conversion follicle scalp goren plucked hair follicle assay predicts response 93 sensitivity 83 specificity tretinoin upregulate booster adjuvant 75 percent regrowth shedding phase adherence twice daily six months wrong diagnosis telogen effluvium iron deficiency thyroid maintenance dependent oral minoxidil switch dht blocker androgenetic alopecia rogaine treatment analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/minoxidil-non-responders.jpg',
  },
  {
    slug         : 'led-face-masks',
    relatedSlugs : ['red-light-therapy-hair-growth', 'chemical-vs-mineral-sunscreen', 'retinoid-ladder'],
    url          : '/articles/skincare/2026/08/led-face-masks',
    title        : 'LED Face Masks: Does At-Home Light Therapy Actually Work?',
    deck         : 'LED face masks promise clearer skin and fewer wrinkles from light alone. The photobiomodulation science is real, but the gap between clinic devices and a home mask is the catch — what the evidence shows for acne and aging.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Treatment Analysis',
    tags         : ['led-face-mask', 'light-therapy', 'photobiomodulation', 'red-light', 'blue-light', 'near-infrared', 'acne', 'anti-aging', 'collagen', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-14',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'led face mask light therapy at home photobiomodulation pbm red light blue light near infrared nir wavelength 415 630 660 830 nanometer cytochrome c oxidase mitochondria atp collagen fibroblast cutibacterium acnes c acnes porphyrins acne vulgaris wrinkles fine lines photoaging skin rejuvenation texture elasticity intradermal collagen density irradiance dose biphasic response consumer device professional clinic grade adjunct rogaine glow sculpting pores meta-analysis systematic review rct dermatology treatment analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/led-face-masks.jpg',
  },
  {
    slug         : 'gua-sha-facial-tools',
    relatedSlugs : ['caffeine-eye-products', 'microneedling-hair-growth', 'skin-cycling'],
    url          : '/articles/skincare/2026/08/gua-sha-facial-tools',
    title        : 'Gua Sha and Facial Tools: What Can They Actually Do for Your Skin?',
    deck         : 'Gua sha stones and jade rollers promise to sculpt your face, drain lymph, and boost collagen. The de-puffing is real and temporary; the sculpting and collagen claims are not supported — what the evidence shows.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Treatment Analysis',
    tags         : ['gua-sha', 'jade-roller', 'facial-roller', 'facial-massage', 'lymphatic-drainage', 'de-puffing', 'face-sculpting', 'collagen', 'microcirculation', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-17',
    verdict      : 'Claim Overstated',
    verdictClass : 'partial',
    searchText   : 'gua sha jade roller rose quartz facial tool massage stone scraping lymphatic drainage de-puffing puffiness fluid interstitial microcirculation blood flow glow sculpt lift jawline contour define wrinkles fine lines collagen stimulation structural bone fat pad muscle petechiae bruising melasma inflammatory acne friction infection tiktok viral snatched face temporary transient not supported systematic review mechanical facial rejuvenation dermatology caffeine eye products microneedling treatment analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/gua-sha-facial-tools.jpg',
  },
  {
    slug         : 'skin-cycling',
    relatedSlugs : ['retinol-paradox', 'retinoid-ladder', 'salicylic-acid-concentration'],
    url          : '/articles/skincare/2026/08/skin-cycling',
    title        : 'Skin Cycling: Does the Four-Night Routine Hold Up?',
    deck         : 'Skin cycling is the viral four-night routine of exfoliate, retinoid, then two recovery nights. A review of why the underlying principle is sound, and why the specific schedule itself has never been tested.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Treatment Analysis',
    tags         : ['skin-cycling', 'retinoid', 'exfoliation', 'aha', 'bha', 'skin-barrier', 'whitney-bowe', 'routine', 'recovery-nights', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-14',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'skin cycling four night routine exfoliation night retinoid night recovery nights whitney bowe tiktok viral dermatologist framework aha bha chemical exfoliant retinol tretinoin skin barrier overuse of actives irritation cadence schedule not tested no controlled trial principle sound components proven retinol paradox retinoid ladder ceramide moisturizer barrier repair dermatology times mythbusters allure healthline treatment analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/skin-cycling.jpg',
  },
  {
    slug         : 'centella-asiatica-cica',
    relatedSlugs : ['snail-mucin-skincare', 'ceramide-moisturizers-skin-barrier', 'beauty-peptides-evidence-review'],
    url          : '/articles/skincare/2026/08/centella-asiatica-cica',
    title        : 'Centella Asiatica and "Cica" Skincare: What the Evidence Shows',
    deck         : 'Centella asiatica, sold as \'cica,\' is marketed for soothing, barrier repair, and anti-aging. A review of what the actives actually do, where the evidence is strong, and where the \'cica\' label outruns the science.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['centella-asiatica', 'cica', 'madecassoside', 'asiaticoside', 'gotu-kola', 'tiger-grass', 'skin-barrier', 'soothing', 'wound-healing', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-13',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'centella asiatica cica madecassoside asiaticoside madecassic acid asiatic acid teca cast titrated extract gotu kola tiger grass k-beauty soothing calming anti-inflammatory antioxidant skin barrier repair wound healing burn collagen synthesis angiogenesis anti-aging wrinkle concentration disclosure standardized extract post-procedure cicatrization snail mucin niacinamide ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/centella-cica.jpg',
  },
  {
    slug         : 'low-dose-oral-minoxidil',
    relatedSlugs : ['topical-vs-oral-minoxidil', 'minoxidil-female-pattern-hair-loss', 'minoxidil-vs-novogro'],
    url          : '/articles/haircare/2026/08/low-dose-oral-minoxidil',
    title        : 'Low-Dose Oral Minoxidil for Hair Loss: What the Evidence Shows',
    deck         : 'Low-dose oral minoxidil (LDOM) is the off-label pill driving a hair-loss prescribing surge. A review of the real efficacy evidence, why it works, the side effects marketing glosses over, and how it compares to the topical version.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['oral-minoxidil', 'ldom', 'low-dose-oral-minoxidil', 'hair-loss', 'androgenetic-alopecia', 'off-label', 'hypertrichosis', 'vasodilator', 'telehealth', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-12',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'low dose oral minoxidil ldom pill tablet off-label hair loss androgenetic alopecia pattern hair loss male female vasodilator potassium channel opener anagen sulfotransferase absorption non-responder randolph tosti 2020 jaad observational cohort meta-analysis efficacy safety hypertrichosis unwanted hair edema fluid retention heart rate pericardial effusion cardiovascular boxed warning monitoring telehealth prescription systemic drug topical vs oral minoxidil finasteride treatment analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/oral-minoxidil.jpg',
  },
  {
    slug         : 'microneedling-hair-growth',
    relatedSlugs : ['copper-peptides-hair-growth', 'minoxidil-female-pattern-hair-loss', 'topical-vs-oral-minoxidil'],
    url          : '/articles/haircare/2026/08/microneedling-hair-growth',
    title        : 'Microneedling and Dermarolling for Hair Growth: What the Evidence Shows',
    deck         : 'Microneedling and dermarolling are marketed as a needle-based path to regrowing hair. A review of the wound-healing mechanism, why the most-cited human trial can\'t separate the needling from the minoxidil, and what the evidence supports.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['microneedling', 'dermarolling', 'dermaroller', 'hair-growth', 'hair-loss', 'androgenetic-alopecia', 'wnt', 'minoxidil', 'wound-healing', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-11',
    verdict      : 'Claim Overstated',
    verdictClass : 'partial',
    searchText   : 'microneedling dermarolling dermaroller scalp needling hair growth regrowth androgenetic alopecia wound healing cascade growth factors wnt beta-catenin signaling anagen telogen microchannels minoxidil penetration delivery enhancer adjunct add-on dhurat 2013 international journal of trichology pilot study evaluator blinded combination confound sham controlled monotherapy systematic review meta-analysis needle depth frequency infection scarring copper peptides saw palmetto finasteride treatment analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/microneedling-hair.jpg',
  },
  {
    slug         : 'copper-peptides-hair-growth',
    relatedSlugs : ['beauty-peptides-evidence-review', 'minoxidil-female-pattern-hair-loss', 'finasteride-female-hair-loss'],
    url          : '/articles/haircare/2026/08/copper-peptides-hair-growth',
    title        : 'Copper Peptides (GHK-Cu) for Hair Growth: What the Evidence Shows',
    deck         : 'Copper peptides (GHK-Cu) are marketed as a hair-regrowth active rivaling minoxidil. A review of the follicle-biology mechanism, why the most-cited human study can\'t isolate the peptide, and how the evidence compares to the gold-standard actives.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['copper-peptides', 'ghk-cu', 'tripeptide-copper', 'hair-growth', 'hair-loss', 'dermal-papilla', 'follicle', 'minoxidil', 'finasteride', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-03',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'copper peptides ghk-cu ghk cu glycyl histidyl lysine tripeptide copper-1 hair growth regrowth scalp serum dormant follicle dermal papilla cells growth cycle anagen telogen in vitro preclinical mechanism lee 2016 annals of dermatology 5-aminolevulinic acid 5-ala alavax combination cannot isolate no randomized controlled trial monotherapy minoxidil finasteride gold standard fda approved comparator rosemary oil saw palmetto pickart skin photoaging cosmetic ingredient not approved hair loss drug ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/copper-peptides.jpg',
  },
  {
    slug         : 'growth-factor-egf-serums',
    relatedSlugs : ['exosome-skincare', 'beauty-peptides-evidence-review', 'clinical-trials-beauty-vs-pharma'],
    url          : '/articles/skincare/2026/08/growth-factor-egf-serums',
    title        : 'Growth Factor & EGF Serums: "Regeneration in a Bottle" — What the Evidence Shows',
    deck         : 'Growth-factor and EGF serums promise wound-healing-grade regeneration in a jar. A review of the penetration problem, the industry-funded efficacy studies, the post-procedure evidence, and the unresolved proliferation-safety question.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['growth-factors', 'egf', 'epidermal-growth-factor', 'tns', 'tgf-beta', 'vegf', 'penetration', 'microneedling', 'proliferation-safety', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-02',
    verdict      : 'Claim Overstated',
    verdictClass : 'partial',
    searchText   : 'growth factor egf serums epidermal growth factor tns skin recovery complex regeneration in a bottle wound healing collagen signaling proteins tgf-beta vegf recombinant human rhegf 6045 daltons 500 dalton penetration threshold stratum corneum barrier large molecule cannot penetrate intact skin microneedling adjunct post procedure randomized controlled trial split face two growth factors 20 subjects vehicle effect systematic review aesthetics regenerative medicine industry funded proprietary proliferation cancer safety concern dermatology times misbranding drug claim cosmeceutical peptides exosomes ingredient analysis',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/egf-regeneration-abstract.jpg',
  },
  {
    slug         : 'volufiline-filler-in-a-bottle',
    relatedSlugs : ['pdrn-skin-repair', 'beauty-peptides-evidence-review', 'clinical-trials-beauty-vs-pharma'],
    url          : '/articles/skincare/2026/08/volufiline-filler-in-a-bottle',
    title        : 'Volufiline and the "Filler in a Bottle" Claim: What the Evidence Actually Shows',
    deck         : 'Volufiline is the topical "filler in a bottle" going viral for plumper skin. A review of the sarsasapogenin adipogenesis mechanism, the supplier-run studies behind the volume numbers, and why a cream is not an injectable.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['volufiline', 'sarsasapogenin', 'filler-in-a-bottle', 'adipogenesis', 'dermal-filler', 'sederma', 'penetration', 'anemarrhena', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-08-01',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'volufiline filler in a bottle sarsasapogenin anemarrhena asphodeloides sederma topical plumper skin volume increase 9 percent 28 days 11 percent 56 days adipogenesis preadipocyte differentiation lipid storage fat cells in vitro cell culture us patent 8361516 supplier run studies not peer reviewed manufacturer no independent rct dermal filler injectable hyaluronic acid needle penetration stratum corneum subcutaneous fat harpers bazaar vogue newbeauty viral trend cream is not an injectable ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/fat-cells.jpeg',
  },
  {
    slug         : 'methylene-blue-nootropic',
    relatedSlugs : ['nootropics-category-overview', 'thesis-nootropics-evidence', 'creatine-cognitive-health'],
    url          : '/articles/wellness/2026/08/methylene-blue-nootropic',
    title        : 'Methylene Blue as a Nootropic: What the Evidence Actually Shows',
    deck         : 'Methylene blue is the century-old dye now sold as a viral brain-and-longevity \'brain fuel.\' A review of the real mitochondrial mechanism, why rodent results don\'t translate to human cognition, and the serious serotonin-syndrome risk.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['methylene-blue', 'nootropic', 'biohacking', 'mitochondria', 'longevity', 'serotonin-syndrome', 'maoi', 'g6pd', 'fda', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-28',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'methylene blue nootropic brain fuel biohacking dye 1876 textile methemoglobinemia mitochondria electron battery charger hofseth usc alzheimer clinical trials mixed inconclusive 26 person memory 7 percent brain connectivity maoi monoamine oxidase inhibitor serotonin syndrome ssri snri boxed warning g6pd deficiency hemolysis pregnancy pharmaceutical grade purity heavy metal npr medscape harvard health fda prescribing information rfk longevity anti aging ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/methylene-blue.jpg',
  },
  {
    slug         : 'bpc-157-healing-peptide',
    relatedSlugs : ['peptide-compounding-fda-review', 'compounded-glp1-semaglutide-tirzepatide', 'beauty-peptides-evidence-review'],
    url          : '/articles/pharma/2026/07/bpc-157-healing-peptide',
    title        : 'BPC-157: What the Evidence Actually Shows About the "Healing Peptide"',
    deck         : 'BPC-157 is marketed as a miracle healing peptide for tendons, gut, and injury recovery, and an FDA advisory panel just narrowly backed it for compounding. A review of the striking animal data, the near-absent human evidence, and the regulatory fight.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Ingredient Analysis',
    tags         : ['bpc-157', 'peptides', 'body-protection-compound', 'compounding', '503a', 'pcac', 'fda', 'tendon-healing', 'biohacking', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-29',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'bpc-157 bpc157 body protection compound pentadecapeptide 15 amino acids gastric juice healing peptide tendon muscle ligament gut ulcerative colitis angiogenesis nitric oxide growth factor rats rodents preclinical animal three human pilot studies no randomized controlled trials regeneration or risk narrative review sikiric fda pcac pharmacy compounding advisory committee july 2026 8 6 vote 503a bulk substances list not well characterized adverse events rfk kennedy conflict of interest atlantic time raps nbc research chemical gray market purity ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/bpc-157.jpg',
  },
  {
    slug         : 'cyclospora-outbreak-produce-safety',
    relatedSlugs : ['peanut-allergy-prevention-treatment', 'sublingual-allergy-drops', 'clinical-trials-beauty-vs-pharma'],
    url          : '/articles/pharma/2026/07/cyclospora-outbreak-produce-safety',
    title        : 'The 2026 Cyclospora Outbreak: Should You Stop Eating Produce?',
    deck         : 'A large 2026 Cyclospora outbreak tied to recalled iceberg lettuce has people swearing off salads. A review of what the parasite is, why washing doesn\'t reliably remove it, who is actually at risk, and why avoiding produce is the wrong lesson.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Condition Review',
    tags         : ['cyclospora', 'cyclosporiasis', 'food-safety', 'outbreak', 'iceberg-lettuce', 'recall', 'cdc', 'fda', 'parasite', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-27',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'cyclospora cyclosporiasis cayetanensis parasite outbreak 2026 iceberg lettuce taylor farms taco bell recall july 17 1947 cases nine states 98 hospitalizations michigan ohio indiana kentucky west virginia illinois kansas oklahoma pennsylvania cdc fda surveillance sporulate fecal oral washing rinse 158 degrees cook tmp-smx trimethoprim sulfamethoxazole bactrim immunocompromised should you stop eating produce vegetables consumer reports cidrap nbc food safety condition review',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/cyclospora-lettuce.jpg',
  },
  {
    slug         : 'ai-in-beauty-wellness',
    relatedSlugs : ['dermatologists-beauty-brands', 'clinical-trials-beauty-vs-pharma', 'exosome-skincare'],
    url          : '/articles/wellness/2026/07/ai-in-beauty-wellness',
    title        : 'AI in Beauty and Wellness: What the Evidence Actually Shows',
    deck         : 'AI is marketed as a dermatologist in your pocket, a personalized-formula engine, and a wellness coach. A review of where the technology is genuinely validated, where it is marketing theater, and where it is quietly risky.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Industry Analysis',
    tags         : ['ai', 'skin-analysis', 'personalization', 'generative-ai', 'wellness-bots', 'ftc', 'biometric-privacy', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-23',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'ai artificial intelligence beauty wellness skin analysis scanner esteva nature 2017 dermatologist skin tone bias personalization prose function of beauty generative ai marketing ftc fake review rule 2024 operation ai comply wellness bots therapy chatbot stanford 2025 semigran bmj 2015 symptom checker back end r and d biometric privacy bipa skin360 industry analysis',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/ai-beauty-tech.jpg',
  },
  {
    slug         : 'dermatologists-beauty-brands',
    relatedSlugs : ['ai-in-beauty-wellness', 'clinical-trials-beauty-vs-pharma', 'vitamin-c-serum-stability'],
    url          : '/articles/skincare/2026/07/dermatologists-beauty-brands',
    title        : 'How Dermatologists Interface With Beauty Brands: What the Labels Actually Mean',
    deck         : '\'Dermatologist tested,\' \'dermatologist recommended,\' derm-founded lines, and skinfluencer partnerships all trade on medical authority. A review of what those terms legally mean, who pays whom, and where the credential adds real value.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Industry Analysis',
    tags         : ['dermatologist', 'dermatologist-tested', 'dermatologist-recommended', 'derm-founded', 'skinfluencer', 'nad', 'ftc', 'open-payments', 'endorsement', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-19',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'dermatologist tested recommended developed beauty brands fda definition nad cerave number one dermatologist recommended derm founded dr dennis gross shiseido barbara sturm skinbetter loreal ftc endorsement guides 2023 teami open payments sunshine act feng jama dermatology 2016 skinfluencer disclosure aad guidance medical authority industry analysis',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/dermatologist-beauty-brands.jpg',
  },
  {
    slug         : 'clinical-trials-beauty-vs-pharma',
    relatedSlugs : ['dermatologists-beauty-brands', 'ai-in-beauty-wellness', 'peptide-compounding-fda-review'],
    url          : '/articles/pharma/2026/07/clinical-trials-beauty-vs-pharma',
    title        : 'Clinical Trials in Beauty and Wellness vs Pharma: Why "Clinically Proven" Means Two Different Things',
    deck         : 'A drug\'s \'clinical trial\' and a serum\'s \'clinically proven\' claim share a word and almost nothing else. A review of how pharma trials are actually run, what cosmetic and supplement \'clinical\' testing really is, and how to decode the label.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Industry Analysis',
    tags         : ['clinical-trials', 'clinically-proven', 'clinically-tested', 'fda', 'ind', 'nda', 'gcp', 'clinicaltrials-gov', 'mocra', 'dshea', 'inci', 'perception-panel', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-17',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'clinical trials beauty wellness pharma clinically proven clinically tested intended use cosmetic drug mocra registration inci international nomenclature cosmetic ingredients personal care products council cosing labeling phases ind nda gcp good clinical practice clinicaltrials gov perception panel corneometer tewl contract lab small sample unregistered supplements dshea boots no7 olay niacinamide skinceuticals reyou novogro eurofins loreal ftc 2014 health products compliance guidance 2022 nad decode label industry analysis',
    popular      : true,
    popularRank  : 3,
    readingTime  : 14,
    image        : '/assets/images/clinical-trials-beauty-pharma.jpg',
  },
  {
    slug         : 'exosome-skincare',
    relatedSlugs : ['pdrn-skin-repair', 'beauty-peptides-evidence-review', 'snail-mucin-skincare'],
    url          : '/articles/skincare/2026/07/exosome-skincare',
    title        : 'Exosomes in Skincare: What the Evidence Actually Shows',
    deck         : 'Exosomes are marketed as a next-generation regenerative skincare and post-procedure active. A review of the biology, the FDA\'s warning that no exosome product is approved, the thin clinical evidence, and the safety record.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['exosomes', 'extracellular-vesicles', 'regenerative', 'microneedling', 'stem-cells', 'fda', 'aesthetic', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-14',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'exosomes extracellular vesicles small ev misev2023 regenerative skincare microneedling laser stem cell mesenchymal adipose platelet derived fda unapproved public safety notification adverse events nebraska outbreak jama network open aesthetic surgery journal cureus meta analysis penetration stratum corneum serum skincare ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/neck-skin-closeup.jpg',
  },
  {
    slug         : 'snail-mucin-skincare',
    relatedSlugs : ['glycerin-humectant', 'hyaluronic-acid', 'ceramide-moisturizers-skin-barrier'],
    url          : '/articles/skincare/2026/07/snail-mucin-skincare',
    title        : 'Snail Mucin in Skincare: What the Evidence Actually Shows',
    deck         : 'Snail secretion filtrate is the viral K-beauty hydrator sold on repair and glass-skin claims. A review of what the ingredient actually contains, the small industry-linked trials, and where the evidence holds up.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['snail-mucin', 'snail-secretion-filtrate', 'cosrx', 'k-beauty', 'glycoproteins', 'allantoin', 'hydration', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-13',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'snail mucin snail secretion filtrate ssf cornu aspersum helix aspersa cryptomphalus cosrx 96 percent essence k-beauty glass skin glycoproteins hyaluronic acid glycolic acid allantoin zinc fabi 2013 lim 2020 photoaging tewl hydration elasticity firmness wrinkles skincare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/snails-face.jpg',
  },
  {
    slug         : 'chemical-vs-mineral-sunscreen',
    relatedSlugs : ['spf-claims', 'bemotrizinol-fda-approval', 'dermatologists-beauty-brands'],
    url          : '/articles/skincare/2026/07/chemical-vs-mineral-sunscreen',
    title        : 'Chemical vs. Mineral Sunscreen: What the Evidence Says About Safety and Protection',
    deck         : 'Mineral sunscreen is marketed as the \'clean,\' safer choice; chemical filters as more elegant and effective. A review of how each filter type works, the systemic-absorption studies, the coral-reef claims, and whether either is meaningfully better at preventing skin damage.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['sunscreen', 'mineral-sunscreen', 'chemical-sunscreen', 'zinc-oxide', 'titanium-dioxide', 'oxybenzone', 'avobenzone', 'octinoxate', 'grase', 'fda', 'reef-safe', 'spf', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-09',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'chemical mineral sunscreen zinc oxide titanium dioxide oxybenzone avobenzone octinoxate octocrylene homosalate octisalate grase fda systemic absorption matta jama 2019 2020 endocrine disruption reef safe hawaii sb2571 coral bleaching spf application white cast clean beauty skincare ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/chemical-vs-mineral-sunscreens.jpg',
  },
  {
    slug         : 'apple-cider-vinegar-weight-loss-blood-sugar',
    relatedSlugs : ['berberine-weight-loss', 'l-carnitine-evidence-review', 'compounded-glp1-semaglutide-tirzepatide'],
    url          : '/articles/wellness/2026/07/apple-cider-vinegar-weight-loss-blood-sugar',
    title        : 'Apple Cider Vinegar for Weight Loss and Blood Sugar: What the Evidence Actually Shows',
    deck         : 'ACV is marketed as a fat-burning, blood-sugar-balancing daily tonic. A review of the small human trials, the acetic-acid mechanism, the postprandial-glucose data, and the enamel and esophageal risks the gummies don\'t mention.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['apple-cider-vinegar', 'acv', 'acetic-acid', 'weight-loss', 'blood-sugar', 'insulin-sensitivity', 'postprandial-glucose', 'gummies', 'bragg', 'goli', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-08',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'apple cider vinegar acv acetic acid weight loss blood sugar insulin sensitivity postprandial glucose gastric emptying ampk bragg goli gummies kondo 2009 johnston 2004 diabetes care bmj retracted enamel erosion hypokalemia esophageal wellness ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/apple-cider-vinegar.jpg',
  },
  {
    slug         : 'compounded-glp1-semaglutide-tirzepatide',
    relatedSlugs : ['ozempic-hair-loss-glp1', 'bpc-157-healing-peptide', 'peptide-compounding-fda-review'],
    url          : '/articles/pharma/2026/07/compounded-glp1-semaglutide-tirzepatide',
    title        : 'Compounded Semaglutide and Tirzepatide: What the Shortage Loophole Actually Delivered',
    deck         : 'Telehealth brands sold compounded GLP-1s as a cheaper, identical alternative to Ozempic and Zepbound. A review of the 503A/503B compounding pathway, the FDA\'s shortage-list resolution, and what \'compounded\' actually means for safety, dosing, and efficacy.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Treatment Analysis',
    tags         : ['semaglutide', 'tirzepatide', 'glp-1', 'ozempic', 'wegovy', 'zepbound', 'mounjaro', 'compounding', '503a', '503b', 'fda', 'drug-shortage', 'weight-loss', 'pharma'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-07',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'compounded semaglutide tirzepatide ozempic wegovy zepbound mounjaro glp-1 glp1 503a 503b compounding pharmacy outsourcing facility fda drug shortage list shortage resolution enforcement salt form semaglutide sodium semaglutide acetate bioequivalence dosing error telehealth weight loss obesity diabetes pharma treatment analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/compounded-glp1.jpg',
  },
  {
    slug         : 'peanut-allergy-prevention-treatment',
    relatedSlugs : ['sublingual-allergy-drops', 'baby-eczema-condition-review', 'cyclospora-outbreak-produce-safety'],
    url          : '/articles/pharma/2026/06/peanut-allergy-prevention-treatment',
    title        : 'Prevention and Treatment of Peanut Allergy: What the Evidence Actually Shows',
    deck         : 'For decades, US guidelines told parents to avoid peanuts in infancy — advice that likely worsened the allergy epidemic. A review of the AAP avoidance era, the LEAP trials that reversed the guidance, and the first FDA-approved treatments.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Treatment Analysis',
    tags         : ['peanut-allergy', 'food-allergy', 'leap-trial', 'oral-immunotherapy', 'palforzia', 'omalizumab', 'pediatrics', 'nejm'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-01',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'peanut allergy prevention treatment leap trial leap-on leap-trio early introduction oral immunotherapy oit ar101 palforzia omalizumab xolair outmatch niaid guidelines food allergy anaphylaxis pediatrics nejm clinical practice pharma treatment analysis aap avoidance history bamba israel',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/peanut-allergy.jpg',
  },
  {
    slug         : 'bovine-colostrum-supplements',
    relatedSlugs : ['collagen-supplements-evidence', 'omega-3-anti-inflammatory-evidence', 'electrolyte-powders-hydration'],
    url          : '/articles/wellness/2026/06/bovine-colostrum-supplements',
    title        : 'Bovine Colostrum Supplements: What the Clinical Evidence Actually Shows',
    deck         : 'Bovine colostrum is marketed for gut health, immunity, and skin. A review of the bioavailability question, the human trial evidence, and why the local gut effects are better supported than the systemic claims.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['colostrum', 'bovine-colostrum', 'gut-health', 'immunity', 'immunoglobulins', 'lactoferrin', 'supplements', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-24',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'bovine colostrum armra gut health leaky gut intestinal permeability immunity immunoglobulin igg lactoferrin igf-1 growth factors bioavailability digestion supplement athletes recovery upper respiratory tract infection urti skin hair wellness ingredient analysis',
    popular      : true,
    popularRank  : 4,
    readingTime  : 9,
    image        : '/assets/images/bovine-colostrum.jpg',
  },
  {
    slug         : 'electrolyte-powders-hydration',
    relatedSlugs : ['apple-cider-vinegar-weight-loss-blood-sugar', 'collagen-supplements-evidence', 'omega-3-anti-inflammatory-evidence'],
    url          : '/articles/wellness/2026/06/electrolyte-powders-hydration',
    title        : 'Electrolyte Powders: Does Everyone Actually Need Them?',
    deck         : 'LMNT, Liquid I.V., and the electrolyte boom promise better hydration for everyone. A review of the oral rehydration science, sodium intake data, and when electrolyte supplementation actually matters.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['electrolytes', 'hydration', 'lmnt', 'liquid-iv', 'sodium', 'oral-rehydration', 'sports-nutrition', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-26',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'electrolyte powders hydration lmnt liquid iv sodium potassium magnesium oral rehydration therapy ors cellular transport technology sglt1 cotransport sports drink acsm exercise hyponatremia keto natriuresis dietary guidelines cdc fda wellness ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/electrolyte-powders.jpg',
  },
  {
    slug         : 'castor-oil-hair-growth',
    relatedSlugs : ['hair-growth-timelines', 'biotin-hair-growth', 'rosemary-oil-minoxidil'],
    url          : '/articles/haircare/2026/06/castor-oil-hair-growth',
    title        : 'Castor Oil for Hair Growth: What the Evidence Actually Shows',
    deck         : 'Castor oil is one of the most popular home remedies for hair and eyelash growth. A review of the actual clinical evidence, the borrowed mechanisms, and the hair-felting risk the marketing leaves out.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['castor-oil', 'ricinoleic-acid', 'hair-growth', 'eyelashes', 'hair-oils', 'home-remedy', 'haircare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-28',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'castor oil hair growth jamaican black castor oil ricinoleic acid eyelashes eyebrows prostaglandin ep3 receptor pgd2 bimatoprost latisse coconut oil rosemary oil minoxidil hair felting home remedy haircare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/castor-oil-hair.jpg',
  },
  {
    slug         : 'tretinoin-vs-otc-retinol',
    relatedSlugs : ['bakuchiol-vs-retinol', 'retinoid-ladder', 'retinol-paradox'],
    url          : '/articles/pharma/2026/06/tretinoin-vs-otc-retinol',
    title        : 'Tretinoin vs. OTC Retinol: How Big Is the Efficacy Gap?',
    deck         : 'Prescription tretinoin and OTC retinol are both marketed as retinoids for anti-aging and acne. A review of the conversion pathway, the clinical trial evidence, and whether the concentration and formulation claims on OTC products hold up.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['tretinoin', 'retinol', 'retinoids', 'acne', 'anti-aging', 'otc'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-16',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'tretinoin otc retinol efficacy gap retinoids conversion pathway anti-aging acne pharma trial review',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/retinol-tretinoin-comparison.jpg',
  },
  {
    slug         : 'ceramide-moisturizers-skin-barrier',
    relatedSlugs : ['glycerin-humectant', 'creme-de-la-mer-miracle-broth', 'baby-eczema-condition-review'],
    url          : '/articles/skincare/2026/06/ceramide-moisturizers-skin-barrier',
    title        : 'Ceramide Moisturizers and Skin Barrier Claims: What the Evidence Actually Shows',
    deck         : 'Ceramide-containing moisturizers dominate the skincare category, with brands claiming to restore the skin barrier, reduce TEWL, and treat eczema-prone skin. A review of the barrier biology, the clinical evidence, and what the meta-analyses actually show.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['ceramides', 'skin-barrier', 'moisturizer', 'tewl', 'eczema', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-18',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'ceramide moisturizer skin barrier tewl eczema atopic dermatitis cerave skincare ingredient analysis',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/ceramide-moisturizer.jpg',
  },
  {
    slug         : 'tranexamic-acid-hyperpigmentation',
    relatedSlugs : ['niacinamide-concentration', 'azelaic-acid-evidence', 'vitamin-c-serum-stability'],
    url          : '/articles/skincare/2026/06/tranexamic-acid-hyperpigmentation',
    title        : 'Tranexamic Acid for Hyperpigmentation: What the Evidence Actually Shows',
    deck         : 'Tranexamic acid is marketed as a next-generation brightening ingredient for melasma and post-inflammatory hyperpigmentation. A review of the oral and topical trial data, the proposed mechanisms, and how OTC concentrations compare to clinical doses.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['tranexamic-acid', 'hyperpigmentation', 'melasma', 'brightening', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-09',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'tranexamic acid hyperpigmentation melasma brightening post-inflammatory pih oral topical skincare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/tranexamic-acid-serum.jpg',
  },
  {
    slug         : 'omega-3-anti-inflammatory-evidence',
    relatedSlugs : ['lemon-verbena-hibiscus', 'collagen-supplements-evidence', 'bovine-colostrum-supplements'],
    url          : '/articles/wellness/2026/06/omega-3-anti-inflammatory-evidence',
    title        : 'Omega-3 Supplements for Inflammation: What the Evidence Actually Shows',
    deck         : 'Fish oil is among the most widely sold wellness supplements, marketed for inflammation, cardiovascular health, and joint pain. A review of what the clinical evidence actually supports — and where recent large trials have revised expectations downward.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['omega-3', 'fish-oil', 'inflammation', 'cardiovascular', 'supplements', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-11',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'omega-3 fish oil inflammation cardiovascular joint pain epa dha supplement wellness ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/fish-oil-capsules.jpg',
  },
  {
    slug         : 'does-sugar-feed-cancer',
    relatedSlugs : ['berberine-weight-loss', 'apple-cider-vinegar-weight-loss-blood-sugar', 'omega-3-anti-inflammatory-evidence'],
    url          : '/articles/pharma/2026/06/does-sugar-feed-cancer',
    title        : 'Does Sugar Feed Cancer? What the Clinical Evidence Actually Shows',
    deck         : 'The "sugar feeds cancer" claim has driven restrictive diets and an entire wellness industry aimed at oncology patients. The mechanistic story is real — but the dietary translation is not what the trial evidence supports.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['cancer', 'sugar', 'warburg-effect', 'ketogenic-diet', 'oncology', 'nutrition', 'pet-scan', 'cachexia'],
    author       : 'Dr. Ben Ouyang',
    authorSlug   : 'ben-ouyang',
    date         : '2026-06-18',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'sugar cancer warburg effect glucose pet scan fdg ketogenic diet oncology nutrition espen asco wcrf aicr cachexia malnutrition colon cancer ssb sugar sweetened beverages trial review wellness ben ouyang',
    popular      : true,
    popularRank  : 1,
    readingTime  : 10,
    image        : '/assets/images/sugar-cancer-cover.jpg',
  },
  {
    slug         : 'minoxidil-vs-novogro',
    relatedSlugs : ['finasteride-vs-novogro', 'pp405-vs-novogro', 'minoxidil-female-pattern-hair-loss'],
    url          : '/articles/haircare/2026/06/minoxidil-vs-novogro',
    title        : 'Minoxidil vs. NOVOGRO™: Legacy vs. the Future?',
    deck         : 'Minoxidil has been the topical hair loss standard for 40 years. NOVOGRO™ (NV-623 & NV-624) are new AI-discovered molecules targeting dermal papilla cells directly. A review of the preclinical evidence, the mechanism difference, and whether the science justifies the narrative.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['minoxidil', 'novogro','nv-623', 'nv-624', 'nv-273', 'dermal-papilla', 'hair-loss', 'ai-drug-discovery', 'androgenetic-alopecia'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-13',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'minoxidil novogro nv-623 nv-624 nv-273 nv-1065 dermal papilla cells dpc hair follicle organoid ai drug discovery hair loss androgenetic alopecia 5-alpha reductase dht haircare ingredient analysis re:you reyou getreyou biorxiv preprint developing story',
    popular      : true,
    popularRank  : 2,
    readingTime  : 11,
    image        : '/assets/images/minoxidil-vs-novogro-cover.png',
  },
  {
    slug         : 'finasteride-vs-novogro',
    relatedSlugs : ['minoxidil-vs-novogro', 'pp405-vs-novogro', 'saw-palmetto-hair-loss'],
    url          : '/articles/haircare/2026/06/finasteride-vs-novogro',
    title        : 'Finasteride vs. NOVOGRO™ for Female Hair Growth: Is Blocking DHT Without Side Effects Finally Possible?',
    deck         : 'We compare finasteride\'s evidence and safety constraints in women against NV-1065, a non-steroidal DHT-blocking molecule positioned as a follicle-targeted alternative. Part 2 of a 3-part series on NOVOGRO™, a new category of intentionally designed molecules that target damaged hair follicles from the inside and out.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['finasteride', 'novogro', 'nv-623', 'nv-624', 'nv-273','nv-1065', 'female-hair-loss', 'dht', '5-alpha-reductase', 'hair-loss', 'androgenetic-alopecia'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-25',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'finasteride novogro nv-623 nv-624 nv-273 nv-1065 female hair growth female pattern hair loss dht 5-alpha-reductase androgenetic alopecia hair loss non-steroidal anti-androgen dutasteride comparison haircare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/finasteride-vs-novogro-cover.png',
  },
  {
    slug         : 'pp405-vs-novogro',
    relatedSlugs : ['finasteride-vs-novogro', 'minoxidil-vs-novogro', 'saw-palmetto-hair-loss'],
    url          : '/articles/haircare/2026/07/pp405-vs-novogro',
    title        : 'PP405 vs. NOVOGRO™: Which Strategy Makes More Sense?',
    deck         : 'We compare PP405, Pelage Therapeutics\' MPC inhibitor targeting follicle stem cell quiescence, against NV-273, NOVOGRO\'s PHD2 inhibitor targeting the hair follicle environment. Part 3 of a 3-part series on NOVOGRO™, a new category of intentionally designed molecules that target damaged hair follicles from the inside and out.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['pp405', 'novogro', 'nv-273', 'pelage', 'mpc', 'phd2', 'hif1a', 'hair-loss', 'androgenetic-alopecia', 'stem-cells'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-07-06',
    verdict      : 'Promising — Awaiting Clinical Data',
    verdictClass : 'partial',
    searchText   : 'pp405 pelage novogro nv-273 mpc mitochondrial pyruvate carrier phd2 prolyl hydroxylase hif1a hif-1 alpha hair loss stem cells follicle quiescence androgenetic alopecia lactate vegf dermal papilla haircare ingredient analysis re:you reyou getreyou biorxiv preprint developing story',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/skin-histology.jpg',
  },
  {
    slug         : 'bemotrizinol-fda-approval',
    relatedSlugs : ['spf-claims', 'chemical-vs-mineral-sunscreen', 'dermatologists-beauty-brands'],
    url          : '/articles/pharma/2026/06/bemotrizinol-fda-approval',
    title        : 'Bemotrizinol Gets FDA Approval: What the Evidence Says About the First New Sunscreen Ingredient in 20 Years',
    deck         : 'The FDA granted GRASE status to bemotrizinol (BEMT) in June 2026, ending a two-decade drought of new US sunscreen options. A review of the efficacy data, how it compares to existing filters, and what the caveats are.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Regulatory Review',
    tags         : ['bemotrizinol', 'bemt', 'sunscreen', 'fda', 'uva', 'uvb', 'grase', 'photoprotection'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-12',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'bemotrizinol bemt sunscreen fda grase approval uva uvb broad spectrum photoprotection avobenzone oxybenzone zinc oxide titanium dioxide organic filter parsol shield dsm pharma regulatory review',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/bemotrizinol-sunscreen.jpg',
  },
  {
    slug         : 'stress-hair-loss',
    relatedSlugs : ['postpartum-hair-loss', 'telogen-effluvium-treatment', 'ashwagandha-evidence-review'],
    url          : '/articles/haircare/2026/06/stress-hair-loss',
    title        : 'Stress and Hair Loss: The HPA Axis, Cortisol, and What the Evidence Shows',
    deck         : 'Stress is widely blamed for hair loss. A review of the HPA axis and cortisol mechanism, the brain-skin axis research, and how stress-induced shedding differs from pattern hair loss.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Condition Review',
    tags         : ['stress', 'telogen-effluvium', 'cortisol', 'hpa-axis', 'hair-loss', 'adaptogens'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-22',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'stress hair loss cortisol hpa axis telogen effluvium adaptogen ashwagandha brain skin axis sympathetic nervous system hair shedding haircare',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/stress-hair-loss.jpg',
  },
  {
    slug         : 'postpartum-hair-loss',
    relatedSlugs : ['stress-hair-loss', 'telogen-effluvium-treatment', 'pp405-vs-novogro'],
    url          : '/articles/haircare/2026/06/postpartum-hair-loss',
    title        : 'Postpartum Hair Loss: The Hormonal Mechanism and What Actually Helps',
    deck         : 'Postpartum hair loss affects most new mothers. A review of the estrogen withdrawal mechanism, expected timeline, and what the evidence shows for treatments and supplements.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['postpartum', 'hair-loss', 'telogen-effluvium', 'estrogen', 'postnatal-supplements'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-28',
    verdict      : 'Mostly Marketing',
    verdictClass : 'unsupported',
    searchText   : 'postpartum hair loss postnatal vitamins after pregnancy hair loss estrogen withdrawal telogen effluvium new mom shedding biotin collagen haircare',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/postpartum-hair-loss.jpg',
  },
  {
    slug         : 'ketoconazole-hair-loss',
    relatedSlugs : ['finasteride-vs-novogro', 'saw-palmetto-hair-loss', 'pp405-vs-novogro'],
    url          : '/articles/haircare/2026/06/ketoconazole-hair-loss',
    title        : 'Ketoconazole Shampoo for Hair Loss: What the Evidence Actually Shows',
    deck         : 'Ketoconazole 2% shampoo is widely used as an adjunct for androgenetic alopecia. A review of the anti-androgen and antifungal mechanisms, the RCT evidence, and where it fits relative to minoxidil and finasteride.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['ketoconazole', 'nizoral', 'androgenetic-alopecia', 'dht', 'malassezia', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-02',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'ketoconazole shampoo nizoral hair loss androgenetic alopecia dht blocker antifungal malassezia minoxidil finasteride adjunct haircare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/ketoconazole-shampoo.jpg',
  },
  {
    slug         : 'saw-palmetto-hair-loss',
    relatedSlugs : ['finasteride-vs-novogro', 'ketoconazole-hair-loss', 'dht-blockers-evidence'],
    url          : '/articles/haircare/2026/06/saw-palmetto-hair-loss',
    title        : 'Saw Palmetto for Hair Loss: How It Compares to Finasteride',
    deck         : 'Saw palmetto is marketed as a natural finasteride alternative for androgenetic alopecia. A review of the 5-alpha reductase mechanism, clinical trial evidence, and what the 2025–2026 research actually shows.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['saw-palmetto', 'serenoa-repens', 'finasteride', 'dht', 'androgenetic-alopecia', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-05',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'saw palmetto serenoa repens natural finasteride alternative dht blocker 5 alpha reductase androgenetic alopecia hair loss haircare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/saw-palmetto-hair-loss.jpg',
  },
  {
    slug         : 'alopecia-areata-jak-inhibitors',
    relatedSlugs : ['pp405-vs-novogro', 'finasteride-vs-novogro', 'minoxidil-vs-novogro'],
    url          : '/articles/haircare/2026/06/alopecia-areata-jak-inhibitors',
    title        : 'Alopecia Areata and JAK Inhibitors: What the Approvals Actually Mean',
    deck         : 'Baricitinib and ritlecitinib are FDA-approved for alopecia areata. A review of the Phase 3 evidence, what long-term data shows, and how JAK inhibitors compare to older treatments.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Treatment Analysis',
    tags         : ['alopecia-areata', 'jak-inhibitors', 'baricitinib', 'ritlecitinib', 'autoimmune', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-06-08',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'alopecia areata jak inhibitor baricitinib olumiant ritlecitinib litfulo fda approved autoimmune hair loss patchy bald spots treatment haircare',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/alopecia-areata.jpg',
  },
  // ── 3 new articles (May 19, 2026) ────────────────────────────────────────
  {
    slug         : 'thesis-nootropics-evidence',
    relatedSlugs : ['nootropics-category-overview', 'creatine-cognitive-health', 'ashwagandha-evidence-review'],
    url          : '/articles/wellness/2026/05/thesis-nootropics-evidence',
    title        : 'Thesis Nootropics: A Product Breakdown of the Personalized Blend Model',
    deck         : 'Thesis built a $100M+ business on personalized nootropic blends. A breakdown of the four core formulas, the per-ingredient evidence base, the proprietary-blend dose problem, and what the trial literature actually supports.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Product Breakdown',
    tags         : ['nootropics', 'thesis', 'cognitive-supplements', 'caffeine', 'ashwagandha', 'citicoline', 'lion-mane'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-19',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'thesis nootropics product breakdown clarity energy logic motivation citicoline alpha-gpc lion mane ashwagandha caffeine theanine sabroxy 7,8-dihydroxyflavone teacrine dynamine personalized cognitive supplements wellness',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/colorful-pill.jpg',
  },
  {
    slug         : 'creme-de-la-mer-miracle-broth',
    relatedSlugs : ['ceramide-moisturizers-skin-barrier', 'glycerin-humectant', 'dermatologists-beauty-brands'],
    url          : '/articles/skincare/2026/05/creme-de-la-mer-miracle-broth',
    title        : 'Crème de la Mer: A Product Breakdown of the $400 Moisturizer and Its Miracle Broth',
    deck         : 'The original Crème de la Mer sells for over $400 per ounce on the strength of its \'Miracle Broth\' fermentation and a burn-recovery origin story. A breakdown of what\'s actually in the jar, what the evidence shows, and what the price premium reflects.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Product Breakdown',
    tags         : ['la-mer', 'creme-de-la-mer', 'miracle-broth', 'luxury-skincare', 'sea-kelp', 'petrolatum', 'moisturizer'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-19',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'creme de la mer miracle broth max huber sea kelp macrocystis pyrifera petrolatum mineral oil fermentation luxury skincare moisturizer estee lauder product breakdown',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/creme-la-mer.jpg',
  },
  {
    slug         : 'baby-eczema-condition-review',
    relatedSlugs : ['peanut-allergy-prevention-treatment', 'ceramide-moisturizers-skin-barrier', 'glycerin-humectant'],
    url          : '/articles/wellness/2026/05/baby-eczema-condition-review',
    title        : 'Baby Eczema: A Condition Review of Prevalence, Prevention, and the Treatment Ladder',
    deck         : 'Atopic dermatitis affects roughly one in five infants. A review of the skin barrier biology, the disappointing prevention trials (BEEP, PreventADALL), the modern treatment ladder including dupilumab, and what the evidence shows for popular baby eczema products.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Condition Review',
    tags         : ['eczema', 'atopic-dermatitis', 'baby', 'pediatric', 'dupilumab', 'topical-steroids', 'leap-trial', 'beep-trial', 'filaggrin'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-19',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'baby eczema infant atopic dermatitis filaggrin leap trial beep preventadall topical steroids hydrocortisone tacrolimus pimecrolimus crisaborole dupilumab dupixent aveeno mustela cerave national eczema association seal pediatric condition review',
    popular      : false,
    readingTime  : 14,
    image        : '/assets/images/tiny-baby.jpg',
  },
  // ── 2 new articles (May 13, 2026) ────────────────────────────────────────
  {
    slug         : 'beauty-peptides-evidence-review',
    relatedSlugs : ['bakuchiol-vs-retinol', 'retinol-paradox', 'bpc-157-healing-peptide'],
    url          : '/articles/skincare/2026/05/beauty-peptides-evidence-review',
    title        : 'Beauty Peptides by Class: What the Clinical Evidence Actually Shows',
    deck         : 'Matrixyl, GHK-Cu, Argireline, and enzyme-inhibitor peptides are among the most-marketed skincare actives. A class-by-class review of the RCT evidence, penetration data, and the conflict-of-interest landscape that shapes the literature.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['peptides', 'matrixyl', 'ghk-cu', 'argireline', 'cosmeceutical', 'anti-aging'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-05-13',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'beauty peptides matrixyl ghk-cu argireline signal carrier neurotransmitter enzyme inhibitor rct evidence penetration 500 dalton rule cosmeceutical anti-aging skincare ingredient analysis',
    popular      : false,
    readingTime  : 13,
    image        : '/assets/images/women-peptides-skin.jpg',
  },
  {
    slug         : 'peptide-compounding-fda-review',
    relatedSlugs : ['bpc-157-healing-peptide', 'compounded-glp1-semaglutide-tirzepatide', 'beauty-peptides-evidence-review'],
    url          : '/articles/pharma/2026/05/peptide-compounding-fda-review',
    title        : 'Seven Peptides, One Regulatory Gray Zone: Inside the FDA\'s 503A Compounding Review',
    deck         : 'BPC-157, Semax, Epitalon, and four other peptides were removed from the FDA\'s \'do not compound\' list in April 2026. A review of the regulatory history, the evidence gaps that drove the original restrictions, and what the July 2026 PCAC re-review means.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Regulatory Review',
    tags         : ['peptides', 'fda', 'compounding', '503a', 'bpc-157', 'semax', 'epitalon', 'pcac', 'regulatory'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-13',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'peptide compounding fda 503a bulks list bpc-157 kpv tb-500 mots-c emideltide semax epitalon pcac category 2 regulatory review pharma',
    popular      : false,
    readingTime  : 14,
    image        : '/assets/images/peptide-vials.jpg',
  },
  // ── 6 new articles (April 29, 2026) ──────────────────────────────────────
  {
    slug         : 'hair-loss-treatments-sex-differences',
    relatedSlugs : ['finasteride-vs-novogro', 'minoxidil-vs-novogro', 'saw-palmetto-hair-loss'],
    url          : '/articles/haircare/2026/04/hair-loss-treatments-sex-differences',
    title        : 'Hair Loss Treatments in Women vs. Men: What the Evidence Actually Shows',
    deck         : 'Minoxidil, finasteride, and spironolactone are all used for androgenetic alopecia \u2014 but the evidence base differs substantially by sex.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['hair-loss', 'minoxidil', 'finasteride', 'spironolactone', 'aga', 'women', 'men'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'hair loss treatments women men sex differences androgenetic alopecia minoxidil finasteride spironolactone prp ludwig hamilton norwood haircare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/man-woman-hair.jpg',
  },
  {
    slug         : 'hair-growth-timelines',
    relatedSlugs : ['minoxidil-vs-novogro', 'hair-loss-treatments-sex-differences', 'rosemary-oil-minoxidil'],
    url          : '/articles/haircare/2026/04/hair-growth-timelines',
    title        : 'Hair Growth Timelines: What the Anagen Cycle Means for Treatment Expectations',
    deck         : 'Most hair loss treatments require 3\u20136 months before measurable results. The biology of the hair growth cycle explains why \u2014 and why \u201cfast results\u201d marketing is biologically implausible.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Condition Review',
    tags         : ['hair-growth', 'anagen', 'hair-cycle', 'minoxidil', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'hair growth timelines anagen cycle treatment expectations fast results shedding phase minoxidil finasteride haircare condition review',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/woman-hair-loss.jpg',
  },
  {
    slug         : 'biotin-hair-growth',
    relatedSlugs : ['castor-oil-hair-growth', 'hair-growth-timelines', 'rosemary-oil-minoxidil'],
    url          : '/articles/haircare/2026/04/biotin-hair-growth',
    title        : 'Biotin for Hair Growth: Does Routine Supplementation Actually Help?',
    deck         : 'Biotin supplements are among the best-selling hair products on the market. The clinical evidence for supplementation in people without biotin deficiency is nearly absent.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['biotin', 'hair-growth', 'supplements', 'vitamin-b7', 'deficiency'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'biotin hair growth supplementation deficiency keratin rda lab test interference vitamin b7 haircare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/mature-woman-hair.jpg',
  },
  {
    slug         : 'hair-density-vs-thickness',
    relatedSlugs : ['pp405-vs-novogro', 'finasteride-vs-novogro', 'minoxidil-vs-novogro'],
    url          : '/articles/haircare/2026/04/hair-density-vs-thickness',
    title        : 'Hair Density vs. Hair Thickness: Why Studies Can Mislead',
    deck         : '\u201cThicker hair\u201d claims appear in both clinical studies and product marketing \u2014 but density (follicles per cm\u00b2) and diameter (\u03bcm per strand) are distinct measurements that don\u2019t always move together.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['hair-density', 'hair-thickness', 'phototrichogram', 'trichoscan', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'hair density thickness diameter phototrichogram trichoscan coating vs growth study endpoints mislead haircare ingredient analysis',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/senior-woman-hair.jpg',
  },
  {
    slug         : 'salicylic-acid-concentration',
    relatedSlugs : ['azelaic-acid-evidence', 'tretinoin-vs-otc-retinol', 'benzoyl-peroxide-adapalene'],
    url          : '/articles/skincare/2026/04/salicylic-acid-concentration',
    title        : 'Salicylic Acid 0.5% vs. 2%: Does Higher Concentration Improve Outcomes?',
    deck         : 'The FDA OTC acne monograph permits 0.5\u20132% salicylic acid. Whether higher concentrations within that range deliver meaningfully better results is a question the clinical literature has barely addressed.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['salicylic-acid', 'acne', 'bha', 'otc', 'fda', 'concentration'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'salicylic acid concentration 0.5 2 percent acne bha keratolytic comedolytic fda monograph vehicle ph skincare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/acne-skin.jpg',
  },
  {
    slug         : 'caffeine-eye-products',
    relatedSlugs : ['eye-cream-evidence', 'hyaluronic-acid', 'tranexamic-acid-hyperpigmentation'],
    url          : '/articles/skincare/2026/04/caffeine-eye-products',
    title        : 'Caffeine in Eye Products: Is There Any Clinical Evidence It Works?',
    deck         : 'Caffeine is the most commonly cited \u201cactive\u201d in eye serums and creams. The periorbital-specific clinical evidence is limited to small, short-duration studies with modest effect sizes.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['caffeine', 'eye-cream', 'puffiness', 'dark-circles', 'periorbital'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-29',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'caffeine eye products serum cream puffiness dark circles periorbital vasoconstriction phosphodiesterase penetration skincare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/transparent-drops-retinol.jpg',
  },
  {
    slug         : 'retinol-paradox',
    relatedSlugs : ['bakuchiol-vs-retinol', 'tretinoin-vs-otc-retinol', 'beauty-peptides-evidence-review'],
    url          : '/articles/skincare/2026/04/retinol-paradox',
    title        : 'The Retinol Paradox: Does Concentration Actually Predict Efficacy?',
    deck         : 'The serum aisle promises \u201cclinical-strength\u201d at every price point. The randomized trial literature is more equivocal.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['retinol', 'vitamin-a', 'anti-aging'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-09',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'retinol paradox concentration efficacy skincare ingredient analysis vitamin a anti-aging serum clinical strength',
    popular      : false,
    readingTime  : 12,
    image        : '/assets/images/liquid-beauty-product.jpg',
  },
  {
    slug         : 'benzoyl-peroxide-adapalene',
    relatedSlugs : ['adapalene-otc-switch', 'tretinoin-vs-otc-retinol', 'salicylic-acid-concentration'],
    url          : '/articles/pharma/2026/02/benzoyl-peroxide-adapalene',
    title        : 'Benzoyl Peroxide vs. Adapalene: What the 2025 Head-to-Head Trials Actually Show',
    deck         : 'OTC acne marketing has outpaced the clinical evidence. We examine four recent RCTs and assess what\u2019s justified for first-line recommendation.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Ingredient Analysis',
    tags         : ['acne', 'benzoyl-peroxide', 'adapalene', 'otc'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-02-28',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'benzoyl peroxide adapalene acne otc trial review rct head to head 2025 pharma',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/acne-skin.jpg',
  },
  {
    slug         : 'hyaluronic-acid',
    relatedSlugs : ['snail-mucin-skincare', 'glycerin-humectant', 'ceramide-moisturizers-skin-barrier'],
    url          : '/articles/skincare/2026/02/hyaluronic-acid',
    title        : 'Hyaluronic Acid: Proven Humectant or Hydration Theater?',
    deck         : 'Molecular weight matters \u2014 yet almost no consumer product discloses it. A review of transepidermal penetration studies and what they mean for product selection.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Product Breakdown',
    tags         : ['hyaluronic-acid', 'hydration', 'skincare', 'molecular-weight'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-02-10',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'hyaluronic acid humectant hydration theater molecular weight penetration skincare product breakdown',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/hyaluronic-acid-clear.jpg',
  },
  {
    slug         : 'niacinamide-concentration',
    relatedSlugs : ['azelaic-acid-evidence', 'tranexamic-acid-hyperpigmentation', 'vitamin-c-serum-stability'],
    url          : '/articles/skincare/2026/01/niacinamide-concentration',
    title        : 'Niacinamide at 5% vs. 10%: Is There a Real Clinical Difference?',
    deck         : 'Niacinamide has become the industry\u2019s favorite multi-tasking ingredient. Examining sebum-regulation, pigmentation, and barrier-repair endpoints across concentration thresholds.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['niacinamide', 'vitamin-b3', 'pigmentation', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-01-22',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'niacinamide concentration 5 10 percent clinical difference sebum pigmentation barrier skincare ingredient analysis',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/dark-drops-retinol.jpg',
  },
  {
    slug         : 'collagen-peptides',
    relatedSlugs : ['collagen-supplements-skin', 'collagen-supplements-evidence', 'hyaluronic-acid'],
    url          : '/articles/wellness/2026/01/collagen-peptides',
    title        : 'Oral Collagen Peptides: The Bioavailability Question the Marketing Skips',
    deck         : 'A $5 billion supplement category rests on a plausible mechanism \u2014 but does the evidence reach the skin? Dissecting the trials behind the claims.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['collagen', 'supplements', 'bioavailability', 'skin'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-01-08',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'oral collagen peptides bioavailability supplement skin anti-aging pharma product breakdown',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/oral-collagen-abstract.jpg',
  },
  {
    slug         : 'spf-claims',
    relatedSlugs : ['bemotrizinol-fda-approval', 'chemical-vs-mineral-sunscreen', 'dermatologists-beauty-brands'],
    url          : '/articles/skincare/2025/12/spf-claims',
    title        : 'SPF 50 vs. SPF 100: What the FDA Monograph and Real-World Data Say',
    deck         : 'High-SPF marketing implies proportionally higher protection. The logarithmic reality, application thickness variables, and UVA protection gaps are rarely disclosed.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Regulatory Review',
    tags         : ['spf', 'sunscreen', 'fda', 'uva', 'uvb'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2025-12-18',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'spf 50 100 sunscreen fda monograph real world data logarithmic uva uvb skincare regulatory review',
    popular      : false,
    readingTime  : 7,
    image        : '/assets/images/man-sunburn-back.jpg',
  },
  {
    slug         : 'retinoid-ladder',
    relatedSlugs : ['tretinoin-vs-otc-retinol', 'bakuchiol-vs-retinol', 'retinol-paradox'],
    url          : '/articles/skincare/2026/03/retinoid-ladder',
    title        : 'The Retinoid Ladder: Matching Strength to Indication Across OTC and Rx Formulations',
    deck         : 'Retinol, retinaldehyde, adapalene, tretinoin — the marketing hierarchy rarely maps to the clinical evidence hierarchy. A structured look at conversion rates, irritation profiles, and what the RCTs actually show.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['retinoids', 'tretinoin', 'retinol', 'adapalene', 'photoaging'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-03-12',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'retinoid ladder retinol retinaldehyde adapalene tretinoin otc rx skincare ingredient analysis photoaging',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/retinoid-abstract.jpg',
  },
  {
    slug         : 'adapalene-otc-switch',
    relatedSlugs : ['benzoyl-peroxide-adapalene', 'tretinoin-vs-otc-retinol', 'salicylic-acid-concentration'],
    url          : '/articles/pharma/2026/02/adapalene-otc-switch',
    title        : 'Adapalene 0.1% OTC: What the Switch Means for Acne Management and Prescriber Guidance',
    deck         : 'The FDA OTC switch for adapalene 0.1% changed the access landscape for topical retinoids. A review of the regulatory basis, combination data with BPO, and implications for clinical practice.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Regulatory Review',
    tags         : ['adapalene', 'acne', 'fda', 'otc', 'benzoyl-peroxide'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-02-20',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'adapalene otc switch fda acne benzoyl peroxide combination pharma regulatory review',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/adapalene-cream.jpg',
  },
  {
    slug         : 'collagen-supplements-evidence',
    relatedSlugs : ['collagen-supplements-skin', 'collagen-peptides', 'hyaluronic-acid'],
    url          : '/articles/wellness/2026/01/collagen-supplements-evidence',
    title        : 'Collagen Supplements: Parsing the Bioavailability Claims from the Clinical Outcomes Data',
    deck         : 'Hydrolysed collagen peptides are among the fastest-growing supplement categories. A critical review of absorption evidence, skin-outcome RCTs, and what integrative practice guidelines actually recommend.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Product Breakdown',
    tags         : ['collagen', 'supplements', 'bioavailability', 'skin', 'integrative'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-01-30',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'collagen supplements bioavailability hydrolysed peptides skin rct integrative pharma product breakdown',
    popular      : true,
    popularRank  : 5,
    readingTime  : 10,
    image        : '/assets/images/collagen-supplement.jpg',
  },
  // ── Part B: 16 new articles (April 2026) ──────────────────────────────────

  // Skincare S1
  {
    slug         : 'vitamin-c-serum-stability',
    relatedSlugs : ['niacinamide-concentration', 'azelaic-acid-evidence', 'hyaluronic-acid'],
    url          : '/articles/skincare/2026/04/vitamin-c-serum-stability',
    title        : 'Vitamin C Serums: Does Percentage Actually Predict Stability?',
    deck         : 'The serum aisle promises \u201cclinical-strength\u201d vitamin C at every price point. The randomized trial literature on L-ascorbic acid stability, oxidation, and efficacy is more equivocal.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['vitamin-c', 'ascorbic-acid', 'antioxidant', 'skincare'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-07',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'vitamin c serum stability percentage best vitamin c serum skinceuticals ce ferulic l-ascorbic acid oxidation skincare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/macro-orange.jpg',
  },
  // Skincare S2
  {
    slug         : 'azelaic-acid-evidence',
    relatedSlugs : ['salicylic-acid-concentration', 'niacinamide-concentration', 'tretinoin-vs-otc-retinol'],
    url          : '/articles/skincare/2026/04/azelaic-acid-evidence',
    title        : 'Azelaic Acid: The Underrated Ingredient the Evidence Actually Supports',
    deck         : 'Azelaic acid has robust RCT evidence for rosacea, acne, and post-inflammatory hyperpigmentation \u2014 yet remains underrepresented in mainstream skincare marketing.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['azelaic-acid', 'rosacea', 'acne', 'pigmentation', 'skincare'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-10',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'azelaic acid evidence skin care pigmentation rosacea acne hyperpigmentation skincare ingredient analysis',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/man-sunburn-back.jpg',
  },
  // Skincare S3
  {
    slug         : 'bakuchiol-vs-retinol',
    relatedSlugs : ['tretinoin-vs-otc-retinol', 'retinol-paradox', 'retinoid-ladder'],
    url          : '/articles/skincare/2026/04/bakuchiol-vs-retinol',
    title        : 'Bakuchiol vs. Retinol: Is the \u201cNatural Alternative\u201d Claim Justified?',
    deck         : 'Bakuchiol is marketed as a plant-based retinol alternative with comparable efficacy and better tolerability. The clinical trial literature is more limited than the marketing implies.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['bakuchiol', 'retinol', 'retinoids', 'anti-aging', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-12',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'bakuchiol retinol serum retinoids natural alternative skincare ingredient analysis anti-aging',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/pipete-serum-texture.jpg',
  },
  // Skincare S4
  {
    slug         : 'eye-cream-evidence',
    relatedSlugs : ['caffeine-eye-products', 'dermatologists-beauty-brands', 'exosome-skincare'],
    url          : '/articles/skincare/2026/04/eye-cream-evidence',
    title        : 'Eye Creams: Clinically Meaningful or Expensive Moisturizer?',
    deck         : 'Eye creams command premium prices on claims of targeted dark circle and wrinkle reduction. The clinical evidence for periorbital-specific formulations versus regular moisturizers is thin.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Product Breakdown',
    tags         : ['eye-cream', 'dark-circles', 'periorbital', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-14',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'best eye cream eye cream for dark circles hyaluronic acid serum periorbital skincare product breakdown',
    popular      : false,
    readingTime  : 8,
    image        : '/assets/images/transparent-drops-retinol.jpg',
  },
  // Haircare H1
  {
    slug         : 'minoxidil-female-pattern-hair-loss',
    relatedSlugs : ['minoxidil-vs-novogro', 'finasteride-female-hair-loss', 'topical-vs-oral-minoxidil'],
    url          : '/articles/haircare/2026/04/minoxidil-female-pattern-hair-loss',
    title        : 'Minoxidil for Female Pattern Hair Loss: What the Evidence Actually Shows',
    deck         : 'Minoxidil is the only FDA-approved topical treatment for female pattern hair loss. A review of the RCT evidence, dosing considerations, and what the trials show about response rates and limitations.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['minoxidil', 'fphl', 'androgenetic-alopecia', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-02',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'minoxidil female pattern baldness fphl hair loss androgenic alopecia rogaine hair growth women',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/female-pattern-hair-loss.jpg',
  },
  // Haircare H2
  {
    slug         : 'dht-blockers-evidence',
    relatedSlugs : ['saw-palmetto-hair-loss', 'finasteride-vs-novogro', 'finasteride-female-hair-loss'],
    url          : '/articles/haircare/2026/04/dht-blockers-evidence',
    title        : 'DHT Blockers: Finasteride, Saw Palmetto, and the Evidence Gap',
    deck         : 'DHT-blocking supplements and drugs are marketed for androgenetic alopecia. The evidence gap between finasteride and natural DHT blockers like saw palmetto is substantial.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['dht-blocker', 'finasteride', 'saw-palmetto', 'androgenetic-alopecia'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-04',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'dht blocker natural dht blocker minoxidil finasteride androgenic alopecia saw palmetto hair loss',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/dht-blocker.jpg',
  },
  // Haircare H3
  {
    slug         : 'telogen-effluvium-treatment',
    relatedSlugs : ['postpartum-hair-loss', 'stress-hair-loss', 'pp405-vs-novogro'],
    url          : '/articles/haircare/2026/04/telogen-effluvium-treatment',
    title        : 'Telogen Effluvium: What Triggers It and What the Trials Say About Recovery',
    deck         : 'Telogen effluvium is diffuse hair shedding triggered by physiological stress. A review of the evidence on triggers, natural recovery timeline, and what interventions the trials actually support.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Condition Review',
    tags         : ['telogen-effluvium', 'postpartum-hair-loss', 'stress-hair-loss', 'hair-loss'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-06',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'telogen effluvium hair loss postpartum hair loss stress and hair loss diffuse shedding recovery',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/senior-woman-hair.jpg',
  },
  // Haircare H4
  {
    slug         : 'dandruff-scalp-microbiome',
    relatedSlugs : ['ketoconazole-hair-loss', 'pp405-vs-novogro', 'castor-oil-hair-growth'],
    url          : '/articles/haircare/2026/04/dandruff-scalp-microbiome',
    title        : 'Dandruff and the Scalp Microbiome: Does Treating the Fungus Fix the Flaking?',
    deck         : 'Dandruff is linked to Malassezia overgrowth on the scalp. A review of the evidence for antifungal shampoos, scalp microbiome research, and what the RCTs show about treatment efficacy.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['dandruff', 'scalp-microbiome', 'malassezia', 'antifungal'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-08',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'dandruff treatment medicated dandruff shampoo scalp treatment malassezia microbiome ketoconazole zinc pyrithione',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/abstract-microbiome.jpg',
  },
  // Wellness W1
  {
    slug         : 'collagen-supplements-skin',
    relatedSlugs : ['collagen-supplements-evidence', 'collagen-peptides', 'hyaluronic-acid'],
    url          : '/articles/wellness/2026/04/collagen-supplements-skin',
    title        : 'Oral Collagen Supplements: Does Intake Actually Reach the Skin?',
    deck         : 'Collagen supplements are marketed for skin elasticity and wrinkle reduction. A review of the bioavailability evidence, skin-outcome RCTs, and what the absorption data actually show.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['collagen', 'supplements', 'skin', 'bioavailability'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-03',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'collagen liquid collagen collagen supplements skin elasticity wrinkle bioavailability wellness',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/oral-collagen-supplement.jpg',
  },
  // Wellness W2
  {
    slug         : 'vitamin-d-hair-loss',
    relatedSlugs : ['iron-deficiency-hair-loss', 'biotin-hair-growth', 'red-light-therapy-hair-growth'],
    url          : '/articles/wellness/2026/04/vitamin-d-hair-loss',
    title        : 'Vitamin D and Hair Loss: Correlation, Causation, or Supplement Marketing?',
    deck         : 'Low vitamin D is associated with hair loss in observational studies. A review of the evidence for causation, supplementation trials, and what the data actually support for hair loss management.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['vitamin-d', 'hair-loss', 'supplements', 'deficiency'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-05',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'vitamin d hair loss hair loss blood test hair loss causes deficiency supplement wellness',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/vitamin-d.jpg',
  },
  // Wellness W3
  {
    slug         : 'red-light-therapy-hair-growth',
    relatedSlugs : ['castor-oil-hair-growth', 'hair-growth-timelines', 'biotin-hair-growth'],
    url          : '/articles/wellness/2026/04/red-light-therapy-hair-growth',
    title        : 'Red Light Therapy for Hair Growth: Separating Signal from Noise',
    deck         : 'Low-level laser therapy and red light devices are marketed for hair growth. A review of the RCT evidence, FDA clearance basis, and what the photobiomodulation data actually show.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Product Breakdown',
    tags         : ['red-light-therapy', 'lllt', 'hair-growth', 'photobiomodulation'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-11',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'red light therapy for hair growth hair growth brush red light lllt photobiomodulation laser wellness',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/red-light-therapy.jpg',
  },
  // Wellness W4
  {
    slug         : 'iron-deficiency-hair-loss',
    relatedSlugs : ['vitamin-d-hair-loss', 'biotin-hair-growth', 'telogen-effluvium-treatment'],
    url          : '/articles/wellness/2026/04/iron-deficiency-hair-loss',
    title        : 'Iron Deficiency and Hair Loss: What Ferritin Thresholds Actually Mean',
    deck         : 'Iron deficiency is commonly identified in hair loss workups. A review of the ferritin threshold evidence, the causation question, and what the intervention trials show about iron supplementation for hair loss.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['iron', 'ferritin', 'hair-loss', 'deficiency', 'supplements'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-13',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'iron and hair loss hair loss blood work diffuse thinning hair ferritin threshold supplement wellness',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/mature-woman-hair.jpg',
  },
  // Pharma P1
  {
    slug         : 'ozempic-hair-loss-glp1',
    relatedSlugs : ['compounded-glp1-semaglutide-tirzepatide', 'prp-hair-loss-evidence', 'finasteride-female-hair-loss'],
    url          : '/articles/pharma/2026/04/ozempic-hair-loss-glp1',
    title        : 'GLP-1 Drugs and Hair Loss: What the Ozempic/Zepbound Trial Data Shows',
    deck         : 'Hair loss has emerged as a reported side effect of GLP-1 receptor agonists. A review of the trial data, proposed mechanisms, and what the evidence shows about the nature and management of GLP-1-associated hair loss.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['glp1', 'ozempic', 'semaglutide', 'hair-loss', 'weight-loss'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-01',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'ozempic hair loss hair loss glp1 hair loss zepbound weight loss and hair loss semaglutide tirzepatide pharma trial review',
    popular      : false,
    readingTime  : 9,
    image        : '/assets/images/glp1-drug.jpg',
  },
  // Pharma P2
  {
    slug         : 'topical-vs-oral-minoxidil',
    relatedSlugs : ['minoxidil-vs-novogro', 'minoxidil-female-pattern-hair-loss', 'prp-hair-loss-evidence'],
    url          : '/articles/pharma/2026/04/topical-vs-oral-minoxidil',
    title        : 'Topical vs. Oral Minoxidil: Efficacy, Dosing, and the Evidence Base',
    deck         : 'Oral minoxidil at low doses has emerged as an alternative to topical minoxidil for hair loss. A review of the comparative efficacy data, dosing evidence, and clinical considerations.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['minoxidil', 'oral-minoxidil', 'hair-loss', 'androgenetic-alopecia'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-15',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'minoxidil minoxidil alternative rogaine hair growth oral minoxidil topical vs oral pharma trial review',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/topical-vs-oral-minoxidil.jpg',
  },
  // Pharma P3
  {
    slug         : 'finasteride-female-hair-loss',
    relatedSlugs : ['finasteride-vs-novogro', 'saw-palmetto-hair-loss', 'minoxidil-female-pattern-hair-loss'],
    url          : '/articles/pharma/2026/04/finasteride-female-hair-loss',
    title        : 'Finasteride for Female Hair Loss: What the Evidence Supports (and What It Doesn\u2019t)',
    deck         : 'Finasteride is FDA-approved for male androgenetic alopecia but used off-label in women. A review of the evidence for finasteride in female pattern hair loss, dosing considerations, and safety data.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['finasteride', 'fphl', 'androgenetic-alopecia', 'hair-loss'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-17',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'minoxidil finasteride androgenic alopecia in females hair loss dermatologist female pattern hair loss pharma trial review',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/finasteride-pills.jpg',
  },
  // Pharma P4
  {
    slug         : 'prp-hair-loss-evidence',
    relatedSlugs : ['finasteride-female-hair-loss', 'topical-vs-oral-minoxidil', 'pp405-vs-novogro'],
    url          : '/articles/pharma/2026/04/prp-hair-loss-evidence',
    title        : 'PRP for Hair Loss: Reviewing the Randomized Trial Evidence',
    deck         : 'Platelet-rich plasma injections are marketed for hair restoration. A review of the randomized trial evidence, protocol variability, and what the data show about PRP efficacy for androgenetic alopecia.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['prp', 'hair-restoration', 'androgenetic-alopecia', 'hair-loss'],
    author       : 'Dr. S.E. Cho',
    authorSlug   : 'stella-cho',
    date         : '2026-04-19',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'prp hair treatment hair restoration microneedling for hair loss platelet rich plasma pharma trial review',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/prp-treatment.jpg',
  },
  {
    slug         : 'rosemary-oil-minoxidil',
    relatedSlugs : ['hair-growth-timelines', 'castor-oil-hair-growth', 'minoxidil-vs-novogro'],
    url          : '/articles/haircare/2026/04/rosemary-oil-minoxidil',
    title        : 'Rosemary Oil vs. Minoxidil: What the Evidence Actually Shows',
    deck         : 'A single small trial comparing rosemary oil to 2% minoxidil has fuelled a billion-dollar \u201cnatural\u201d hair-growth category. The methodology deserves closer scrutiny.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['rosemary-oil', 'minoxidil', 'hair-growth', 'natural-remedies'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-18',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'rosemary oil minoxidil hair growth natural alternative rogaine essential oil haircare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : 'https://images.unsplash.com/photo-1608571423902-eed4a5ad8108?w=1200',
  },
  {
    slug         : 'bond-builders-hair',
    relatedSlugs : ['pp405-vs-novogro', 'castor-oil-hair-growth', 'finasteride-vs-novogro'],
    url          : '/articles/haircare/2026/04/bond-builders-hair',
    title        : 'Bond Builders (Olaplex, K18): Do They Actually Repair Hair?',
    deck         : 'The bond-repair category commands premium prices on a claim of covalent chemistry. The independent peer-reviewed literature is considerably thinner than the marketing spend.',
    category     : 'Haircare',
    categorySlug : 'haircare',
    type         : 'Ingredient Analysis',
    tags         : ['olaplex', 'k18', 'bond-repair', 'hair-damage'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-18',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'olaplex k18 bond builder bond repair hair damage covalent chemistry haircare ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/young-woman-hair.jpg',
  },
  {
    slug         : 'berberine-weight-loss',
    relatedSlugs : ['apple-cider-vinegar-weight-loss-blood-sugar', 'l-carnitine-evidence-review', 'compounded-glp1-semaglutide-tirzepatide'],
    url          : '/articles/wellness/2026/04/berberine-weight-loss',
    title        : 'Berberine for Weight Loss: A Clinical Review of \u201cNature\u2019s Ozempic\u201d',
    deck         : 'Berberine activates AMPK and lowers blood glucose \u2014 mechanisms it shares with metformin. Whether it warrants the \u201cnatural Ozempic\u201d label is a different question.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['berberine', 'weight-loss', 'ampk', 'metformin', 'supplements'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-18',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'berberine weight loss nature ozempic ampk metformin supplement blood glucose wellness ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/red-berries.jpg',
  },
  {
    slug         : 'magnesium-threonate',
    relatedSlugs : ['creatine-cognitive-health', 'thesis-nootropics-evidence', 'nootropics-category-overview'],
    url          : '/articles/wellness/2026/04/magnesium-threonate',
    title        : 'Magnesium L-Threonate: Does It Actually Cross the Blood-Brain Barrier?',
    deck         : 'The marketing claim rests primarily on rat studies and a single small human trial. Whether the threonate form uniquely elevates brain magnesium levels in humans remains unresolved.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['magnesium', 'threonate', 'brain-health', 'supplements', 'nootropics'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-04-18',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'magnesium l-threonate blood brain barrier magtein brain health nootropic supplement wellness ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : 'https://images.unsplash.com/photo-1559757175-0eb30cd8c063',
  },
  {
    slug         : 'pdrn-skin-repair',
    relatedSlugs : ['exosome-skincare', 'snail-mucin-skincare', 'beauty-peptides-evidence-review'],
    url          : '/articles/skincare/2026/05/pdrn-skin-repair',
    title        : 'PDRN and Skin Repair: What the Regenerative Medicine Evidence Actually Shows',
    deck         : 'PDRN (polydeoxyribonucleotide) is a salmon DNA-derived ingredient used in wound healing and aesthetic medicine. A review of the A2A receptor mechanism, injectable clinical trials, and whether topical skincare can replicate those effects.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['pdrn', 'polydeoxyribonucleotide', 'salmon-dna', 'regenerative', 'skincare', 'aesthetic'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-10',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'pdrn polydeoxyribonucleotide salmon dna skin booster injectable regenerative medicine a2a receptor wound healing aesthetic skincare ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/facial-skin-texture.jpg',
  },
  {
    slug         : 'sublingual-allergy-drops',
    relatedSlugs : ['peanut-allergy-prevention-treatment', 'cyclospora-outbreak-produce-safety', 'clinical-trials-beauty-vs-pharma'],
    url          : '/articles/pharma/2026/05/sublingual-allergy-drops',
    title        : 'Sublingual Allergy Drops vs. Allergy Shots: What the Evidence Says About SLIT',
    deck         : 'Sublingual immunotherapy drops are marketed as a convenient at-home alternative to allergy shots. A review of the RCT evidence, FDA approval status of tablets vs. compounded drops, and safety profile compared to SCIT.',
    category     : 'Pharma & OTC',
    categorySlug : 'pharma',
    type         : 'Trial Review',
    tags         : ['slit', 'sublingual-immunotherapy', 'allergy-drops', 'allergy-shots', 'scit', 'immunotherapy'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-11',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'sublingual allergy drops slit scit immunotherapy tablets grastek ragwitek odactra oralair compounded drops allergy shots pharma treatment analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/sublingual-allergy-drops.jpg',
  },
  {
    slug         : 'nootropics-category-overview',
    relatedSlugs : ['thesis-nootropics-evidence', 'creatine-cognitive-health', 'ashwagandha-evidence-review'],
    url          : '/articles/wellness/2026/05/nootropics-category-overview',
    title        : 'Nootropics: Which Common Ingredients Have Evidence Behind Them?',
    deck         : 'The nootropics category spans well-studied compounds to unproven blends. An evidence-tiered review of caffeine, L-theanine, citicoline, lion\u2019s mane, bacopa, ashwagandha, and ginkgo biloba, ranked by strength of clinical trial support.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['nootropics', 'cognitive-enhancement', 'caffeine', 'l-theanine', 'bacopa', 'ashwagandha', 'lions-mane', 'ginkgo', 'citicoline'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-11',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'nootropics cognitive enhancement caffeine l-theanine citicoline cdp-choline lions mane hericium bacopa monnieri ashwagandha ginkgo biloba phosphatidylserine huperzine wellness category overview',
    popular      : false,
    readingTime  : 13,
    image        : '/assets/images/nootropics-abstract.jpg',
  },
  {
    slug         : 'glycerin-humectant',
    relatedSlugs : ['ceramide-moisturizers-skin-barrier', 'snail-mucin-skincare', 'creme-de-la-mer-miracle-broth'],
    url          : '/articles/skincare/2026/05/glycerin-humectant',
    title        : 'Glycerin in Skincare: Humectant Powerhouse or Overhyped Filler?',
    deck         : 'Glycerin is one of the most common ingredients in moisturizers and serums. A review of its humectant mechanism, stratum corneum hydration and barrier-function evidence, concentration effects, and comparison with hyaluronic acid, urea, and propylene glycol.',
    category     : 'Skincare & Beauty',
    categorySlug : 'skincare',
    type         : 'Ingredient Analysis',
    tags         : ['glycerin', 'glycerol', 'humectant', 'moisturizer', 'skin-barrier', 'hydration', 'skincare'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-11',
    verdict      : 'Supported',
    verdictClass : 'supported',
    searchText   : 'glycerin glycerol humectant moisturizer skin barrier hydration aquaporin aqp3 stratum corneum tewl hyaluronic acid urea propylene glycol skincare ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/glycerin.jpg',
  },

  {
    slug         : 'ashwagandha-evidence-review',
    relatedSlugs : ['l-carnitine-evidence-review', 'creatine-cognitive-health', 'nootropics-category-overview'],
    url          : '/articles/wellness/2026/05/ashwagandha-evidence-review',
    title        : 'Ashwagandha for Athletic Performance and Testosterone: What the Evidence Actually Shows',
    deck         : 'Ashwagandha is marketed for testosterone, muscle strength, and athletic performance. A clinical review of the KSM-66 and Sensoril trial data, the testosterone claim, and what the stress and cortisol evidence actually supports.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['ashwagandha', 'withania-somnifera', 'adaptogens', 'testosterone', 'athletic-performance', 'cortisol', 'ksm-66', 'sensoril', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-20',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'ashwagandha withania somnifera adaptogen testosterone athletic performance cortisol ksm-66 sensoril strength muscle recovery stress wellness ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/ashwagandha.jpg',
  },

  {
    slug         : 'creatine-cognitive-health',
    relatedSlugs : ['magnesium-threonate', 'thesis-nootropics-evidence', 'nootropics-category-overview'],
    url          : '/articles/wellness/2026/05/creatine-cognitive-health',
    title        : 'Creatine for Brain Health: What the Cognitive Evidence Actually Shows',
    deck         : 'Creatine is marketed as a cognitive enhancer and brain fuel supplement. A review of the working memory, mental fatigue, and neuroprotection trial data — and why the benefit is population-specific.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['creatine', 'creatine-monohydrate', 'cognitive-health', 'nootropics', 'brain-health', 'working-memory', 'mental-fatigue', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-21',
    verdict      : 'Partially Supported',
    verdictClass : 'partial',
    searchText   : 'creatine monohydrate cognitive health brain fuel working memory mental fatigue neuroprotection vegetarians older adults sleep deprivation wellness ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/creatine.jpg',
  },

  {
    slug         : 'l-carnitine-evidence-review',
    relatedSlugs : ['apple-cider-vinegar-weight-loss-blood-sugar', 'ashwagandha-evidence-review', 'berberine-weight-loss'],
    url          : '/articles/wellness/2026/05/l-carnitine-evidence-review',
    title        : 'L-Carnitine for Fat Burning and Performance: What the Evidence Shows',
    deck         : 'L-carnitine is sold as a fat-burning and performance supplement. A review of the muscle uptake requirement, the weight loss meta-analysis data, and why the standard supplement protocol fails the mechanism.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['l-carnitine', 'levocarnitine', 'lclt', 'fat-burning', 'weight-loss', 'athletic-performance', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-22',
    verdict      : 'Claim Unsupported',
    verdictClass : 'unsupported',
    searchText   : 'l-carnitine levocarnitine lclt fat burning weight loss athletic performance muscle uptake carbohydrate co-ingestion tmao wellness ingredient analysis',
    popular      : false,
    readingTime  : 11,
    image        : '/assets/images/l-carnitine.jpg',
  },

  {
    slug         : 'lemon-verbena-hibiscus',
    relatedSlugs : ['omega-3-anti-inflammatory-evidence', 'ashwagandha-evidence-review', 'berberine-weight-loss'],
    url          : '/articles/wellness/2026/05/lemon-verbena-hibiscus',
    title        : 'Lemon Verbena and Hibiscus for Joint Health: Evaluating the Combination Evidence',
    deck         : 'Two botanicals with distinct phytochemical profiles are increasingly paired in joint supplements. We review what each ingredient\'s evidence base actually looks like — and what the combination trials add.',
    category     : 'Wellness',
    categorySlug : 'wellness',
    type         : 'Ingredient Analysis',
    tags         : ['lemon-verbena', 'aloysia-citrodora', 'hibiscus', 'hibiscus-sabdariffa', 'joint-health', 'inflammation', 'verbascoside', 'anthocyanins', 'wellness'],
    author       : 'Editorial Team',
    authorSlug   : 'editorial',
    date         : '2026-05-23',
    verdict      : 'Mixed Evidence',
    verdictClass : 'mixed',
    searchText   : 'lemon verbena aloysia citrodora hibiscus sabdariffa joint health inflammation verbascoside anthocyanins womac osteoarthritis uric acid wellness ingredient analysis',
    popular      : false,
    readingTime  : 10,
    image        : '/assets/images/lemon-verbena.jpg',
  },
];

window.dispatchEvent(new CustomEvent('articlesLoaded'));
